...
`/settings` の「EDINET DB 設定」カードから第三者サービス edinetdb.jp の `api_key`（`X-API-Key`）と契約プランを編集する（DB＝`edinetdb_config`・公式 EDINET の env `edinet_api_key` とは別系統）。`api_key` は GET でマスク・更新は **write-only**（空送信は据え置き）。疎通テストは `POST /diagnostics/edinetdb-test`（`{configured,ok,detail}`・detail に月残予算）。#2 売掛/在庫の質の財務取得に使う（[ADR-064](decisions.md)）。
