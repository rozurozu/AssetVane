...
- **レスポンスの起票フィードバック**: `journal_id: number | null`（submit_journal で日記に残せた id・[ADR-029](decisions.md)）／`card_ids: number[]`（propose_card が起票した知識ノート draft の id・[ADR-065](decisions.md)）／`watchlist_candidates: [{ code, company_name, reason }]`（propose_watchlist が提示したウォッチ候補・[ADR-080](decisions.md)）。いずれも呼ばれなければ空/`null`。`watchlist_candidates` は UI がチェックリストで見せ、ユーザーが選んで `POST /watchlist` する（**追加は UI 側＝AI は watchlist を書かない**・surfacing は昼 `/chat` だけで夜 nightly は no-op）。
- **送信中キャンセル**（[ADR-072](decisions.md)）: frontend が送信中に fetch を abort するとサーバは `is_disconnected` で切断を検知し、LLM ループごと打ち切る。切断時は**副作用（journal/proposals/cards の永続化）に到達しない**（末尾集約）ので中途半端な起票は残らない。frontend は中止時に直前の発話を入力欄に戻して編集・送り直しできる（openai 互換経路が対象）。
...
| GET | `/dossiers/{code}` | 銘柄の調査レポート（markdown）と要約ソース一覧（未調査でも 200＋空ドシエ＝`summary_md: ""`＋`last_investigated_at: null`・確定・phase4-spec §5.2）。`investigating: bool`＝今その銘柄を調査中か（プロセスメモリ由来・リロードで「調査中」を保つ＝[ADR-076](decisions.md)）|
...
| メソッド | パス | 用途 |
|---|---|---|
| GET | `/health` | 死活監視・必須環境変数の充足チェック（`llm_cost`＝[ADR-028](decisions.md) コスト状態・`jquants`＝`{plan, delay_days, configured}` で右上バッジを DB プラン由来で動的表示＝[ADR-061](decisions.md)。`delay_days` は `_PLAN_DELAY_DAYS` 定数＝free 84/12週・有料 0） |
...
| POST | `/valuation/backfill-net-cash` | 清原式ネットキャッシュの全銘柄バックフィル起動（sec_code→edinet_code スイープ＋全普通株 net_cash 焼き込みを full_backfill で `run_jobs`・202／free プラン・未設定は 400／実行中は 409・進捗は `/batch/status`・[ADR-083](decisions.md)）|
...
| POST | `/diagnostics/edinetdb-test` | edinetdb.jp 認証ピング（接続値は DB 解決・`{configured,ok,detail}`・[ADR-064](decisions.md)）|
| POST | `/diagnostics/edinet-test` | 公式 EDINET 認証ピング（接続値は DB 解決・当日書類一覧を 1 発・`{configured,ok,detail}`・[ADR-087](decisions.md)）|
...
## 11b. EDINET DB 設定（edinetdb.jp の api_key・プラン・[ADR-064](decisions.md)）

`/settings` の「EDINET DB 設定」カードから第三者サービス edinetdb.jp の `api_key`（`X-API-Key`）と契約プランを編集する（DB＝`edinetdb_config`・公式 EDINET の DB `edinet_config`／[ADR-087](decisions.md) とは別系統）。`api_key` は GET でマスク・更新は **write-only**（空送信は据え置き）。疎通テストは `POST /diagnostics/edinetdb-test`（`{configured,ok,detail}`・detail に月残予算）。#2 売掛/在庫の質の財務取得に使う（[ADR-064](decisions.md)）。

| メソッド | パス | 説明 |
|---|---|---|
| GET | `/edinetdb/config` | 接続の現在値（`{api_key_masked, has_api_key, plan, configured}`） |
| PUT | `/edinetdb/config` | 更新（`{api_key?, plan?}`・`api_key` 空送信は据え置き・`plan`＝`free`/`pro`） |
| POST | `/diagnostics/edinetdb-test` | edinetdb.jp 認証ピング（`/companies` 1 件・`{configured,ok,detail}`） |

- **`plan`** はレート目安（`services/edinetdb_config.py` の `_PLAN_LIMITS`＝free は日100/月600）を決めるが、**実予算の enforce はレスポンスの `x-ratelimit-*-remaining` ヘッダ**で行う（[ADR-064](decisions.md)）。
- 未設定（`configured=false`）では #2 取得は静かに skip（公式 EDINET の `JQuantsError` 落ちと違い通知しない・[ADR-064](decisions.md)）。

## 11c. EDINET（公式）設定（api.edinet-fsa.go.jp の api_key・[ADR-087](decisions.md)）

`/settings` の「EDINET（公式）設定」カードから公式 EDINET（金融庁 EDINET API v2）の `api_key`（Subscription-Key）を編集する（DB＝`edinet_config`・第三者 edinetdb.jp の `edinetdb_config` とは別系統）。旧・env `EDINET_API_KEY` を DB+WebUI へ移管（jquants/edinetdb と同型＝[ADR-061](decisions.md)/[ADR-064](decisions.md)）。`api_key` は GET でマスク・更新は **write-only**（空送信は据え置き）。**plan 概念は無い**（公式 EDINET は回数クォータ無し・レート制限のみ）。有報「事業の内容」テキスト源（テーマタグ段階C・[ADR-056](decisions.md)）に使う。

| メソッド | パス | 説明 |
|---|---|---|
| GET | `/edinet/config` | 接続の現在値（`{api_key_masked, has_api_key, configured}`） |
| PUT | `/edinet/config` | 更新（`{api_key?}`・`api_key` 空送信は据え置き） |
| POST | `/diagnostics/edinet-test` | 公式 EDINET 認証ピング（当日書類一覧を 1 発・`{configured,ok,detail}`） |

- 未設定（`configured=false`）では段階C 取得（`fetch_edinet_descriptions`）は静かに skip（[ADR-018](decisions.md)）。初回は `/settings` で登録するまで動かない。第三者 edinetdb.jp の `edb_` キーをここに入れないこと（疎通テストが `metadata.status` 異常応答で NG を返す＝貼り間違い検知）。

## 12. 知識カード（[ADR-062](decisions.md)）

`/cards` 管理画面から AI アドバイザーの知識カードを追加・編集・削除する。**追加は本文（＋出所 URL）だけ**で、追加時に同期で AI（`assist_card`＝triage 面）が `title`/`when_to_apply`/`level` を生成しつつ verdict で `status` を振り分ける（雑追加リデザイン・[ADR-062](decisions.md) 追補）。`active` 候補は draft 留置で人間が active 化する（active 化＝本番助言に効く＝人間が最終承認・[ADR-009](decisions.md)）。注入対象は `status='active'`。規律は CORE・一般教科書知識は LLM に任せ、ここには「非自明な知識（市場文脈・外部メモ・手法の解釈）」だけ置く。
...
- **`CardOut`** = `{id, title, body, when_to_apply, status, level, sector17_code, theme, market, code, quant_note, always_inject(bool), weight(float・既定1.0), source, triage_reason, embedded_at, created_at, updated_at}`（`embedding` BLOB は返さない・旧 `linked_signal_type` は [ADR-075](decisions.md)・`0035` で DROP）。`weight` は retrieval ランク/注入順を `distance/weight` で重み付け（古い/信頼度低を下げて生かす・[ADR-062](decisions.md) 追補）。`triage_reason` は AI 審査の判定理由（`null`＝AI 未整形）。
- **`status`** = `draft`/`active`/`needs_quant`（計算未実装で実装待ち）/`to_core`（規律→CORE 誘導）/`rejected`（LLM 一般知識でカード不要）。
- **追加/再整形時の verdict 反映**: `needs_quant`/`to_core`/`rejected` はその status を自動反映、`active` は draft 据え置き（人間承認待ち）。AI 面未設定/応答不正は AI 未整形＝status 据え置き（`/cards` は draft・`/cards/{id}/assist` は `triage=null`・[ADR-018](decisions.md)）。
