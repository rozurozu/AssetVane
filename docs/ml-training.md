...
> **状態（2026-06-06・ドラフト）**: 推論側のコード（`features.py`/`infer.py`/`model_store.py`/
> `batch/jobs/score_ai_alpha.py`）と学習コード（`quant/ml/train.py`）は実装済み。本書は**確定済みの
> 特徴量・ラベル・抽出・配置の手順**を記す。`【実測】` の付いた値（ハイパラ確定値・CV 評価値・
> データ量）は、別 PC で初回学習を回したときに**穴埋めする**（phase5-spec.md §3.5・完了条件 4）。
...
- 学習データの財務バックフィルは別 PC 側で行ってよい（ラズパイの financials は保有＋watchlist 限定でも
  ...
  J-Quants **Light 以上**で期間を延ばす（`BACKFILL_YEARS` 相当を別 PC で広げる）。`【実測】データ量`。
...
-- ベンチ＝TOPIX 終値（index_quotes の symbol='^TPX'・Stooq）。
...
`train.py` はライブラリ関数（CLI を持たない）。別 PC で下記のような薄いスクリプトを書いて回す。
...
### ハイパラ（`【実測】` で確定値に置換）
`train.py` の `_DEFAULT_PARAMS`（出発値）: `n_estimators=300 / learning_rate=0.03 / num_leaves=31 /
min_child_samples=20 / subsample=0.8 / colsample_bytree=0.8 / random_state=42`。
- `【実測】` 別 PC で **walk-forward 時系列 CV**（未来を学習に混ぜない）して確定したハイパラをここに記録。
- CV は point-in-time（時系列順・リーク無し）。`random_state` を固定して再現性を担保（ADR-016）。

### 評価指標（`【実測】`）
- 回帰: **RMSE** と **IC（情報係数＝予測と実現超過リターンの spearman 順位相関）**。`【実測】CV の RMSE/IC`。
- （分類にした場合のみ）AUC。
