...
>
> **運用注記（2026-07-01・ユーザー協議）**: この Mac コンテナ学習は初回ベースライン。**運用を Mac 学習だけに固定しない**——大規模データや GPU が効くハイパラ探索などスペックが要る再学習は GPU 搭載ゲーミングPC で回す（ADR-006「学習は別 PC」の精神を維持し、Mac コンテナはその一手段）。
