...
| AI | `advisor_turns` | AI 判断軌跡の観測台帳（1 LLM ターン＝1 行・Tool 列＋規律 2 列・[ADR-092](decisions.md)・0044）| Tool ループ 5 面 |
...
### `advisor_turns` — AI 判断軌跡の観測台帳（[ADR-092](decisions.md)・0044_advisor_turns）
`track_record`（[ADR-077](decisions.md)＝結果の質）と対で「AI が実際にどう判断したか（プロセスの質）」を残す層。Tool ループ（`run_tool_loop`）を通る **5 面**（chat/nightly/reviewer/profiler/skeptic）の 1 ターンを 1 行に焼く（`generate_once` の dossier/tagger/triage はループを通らず自然に対象外）。フック `_record_turn` は `run_tool_loop` の単一出口で 1 回だけ呼ばれ、**`llm._record_usage` と同型**＝独立接続（`get_engine().begin()`）で best-effort insert・失敗は握って LLM 応答を壊さない。**reply 本文は残さず** `tool_sequence`（呼んだ Tool 名＋引数・結果値なし＝[ADR-025](decisions.md)/[ADR-029](decisions.md)）だけ焼く。集計に効く規律 2 列（`called_propose_trade`/`propose_trade_disciplined`）だけ非正規化し（`proposal_outcomes.conviction`/`hit` の前例＝`AVG` で純 SQL 集計）、「submit_journal を呼んだか」等の表示専用フラグは列にせず `GET /advisor/turns` が `tool_sequence` から read-time 導出する。

| 列 | 型 | 説明 |
|---|---|---|
| `id` | INTEGER PK | |
| `created_at` | TEXT | ISO8601 UTC（面別サマリの起点）|
| `source` | TEXT | `'chat'`/`'nightly'`/`'reviewer'`/`'profiler'`/`'skeptic'` |
| `model` | TEXT | 面別に解決された実 model（監査用・[ADR-058](decisions.md)）|
| `tool_sequence` | TEXT | JSON `[{name, args}]`（結果値なし・[ADR-025](decisions.md)）|
| `n_rounds` | INTEGER | Tool 往復回数 |
| `truncated` | INTEGER | 1=`max_rounds` 打ち切り |
| `called_propose_trade` | INTEGER | 1=このターンで `propose_trade` を呼んだ |
| `propose_trade_disciplined` | INTEGER | NULL=非該当 / 1=呼んだ全件が conviction+invalidation+catalyst+horizon 全備 / 0=欠けあり |

- インデックス `(source, created_at)`（面別サマリ・直近降順）。`summarize_propose_trade_discipline`（journaling）が既存 `_extract_trade_proposals`＋`_normalize_conviction/_normalize_horizon` を再利用して規律 2 列を決める（tool_runs 解釈の単一の真実・drift 回避）。
