...
| AI | `general_news` | 一般ニュース台帳（銘柄に紐づかない・カテゴリ別・[ADR-034](decisions.md)）| 夜間ジョブ `fetch_general_news` |
...
> **通貨について**: 当面は日本株のみで全て JPY 前提。`daily_quotes`/`holdings`/`cash` に通貨列は持たない。**米国株（USD 建て）が入る Phase 7 で、通貨列と FX 換算を導入する**（[roadmap.md Phase 7](roadmap.md)・[ADR-010](decisions.md)）。それまで通貨は YAGNI。**スクリーナー（[ADR-031](decisions.md)）も同じ理由で市場ごとに分離**し、米株は Phase 7 で `/us-stocks` 別ルート・別 `valuation_snapshots` 相当を作る（¥と$の時価総額混在・33業種とGICS跨ぎの相対ランクが無意味になるのを避ける）。
...
- **日本株専用**。米株は Phase 7 で別スナップショット・`/us-stocks`（通貨/GICS の境界・[roadmap.md Phase 7](roadmap.md)）。
...
### `general_news` — 一般ニュース台帳（銘柄に紐づかない・[ADR-034](decisions.md)）
銘柄に紐づかない一般ニュース（市況・マクロ・世界情勢）を**カテゴリ別**に持つ。`dossier_sources`（`code` FK 必須）とは住所が違うため**別テーブル**にし、`code` FK を持たず `category` 列を持つ（[ADR-034](decisions.md)）。**記事全文は持たず要約と URL だけ**残す（[ADR-020](decisions.md) の流儀）。夜間ジョブ `fetch_general_news`（`run_advisor` 直前）が `NewsAdapter.fetch_general_news()` で取得・要約して UPSERT する。

| 列 | 型 | 説明 |
|---|---|---|
| `id` | INTEGER PK | |
| `category` | TEXT | カテゴリ表示ラベル（市況／マクロ／世界情勢 等）|
| `url` | TEXT | ソース URL（**UNIQUE**＝重複防止）|
| `title` | TEXT | 見出し（任意）|
| `summary` | TEXT | 短い要約（本文は保存しない）|
| `published_at` | TEXT | 発行日 |
| `fetched_at` | TEXT | 取り込んだ日時 |
| `source_type` | TEXT | `news` 等（将来拡張）|
| `extraction_status` | TEXT | 取得レベル `summarized`/`description`/`headline` |

- インデックス: `url`（UNIQUE）、`category`。`url` UNIQUE ＋ `on_conflict_do_nothing` で冪等。
- 取り込みは「発行が lookback 日以内・URL 重複排除・カテゴリあたり件数上限」。カテゴリ定義・件数上限・lookback は定数モジュール `app/adapters/general_news_config.py`（env 化しない＝[ADR-034](decisions.md)）。
- 消費先は `GET /general-news`（Dashboard widget）と Tool `get_general_news`（軸1 briefing／軸2 チャット）。
