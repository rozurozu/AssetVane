...
> **留意点（初回バックフィル × レート制限）**: Free は **5 req/分**。銘柄ごとに 1 リクエストで全銘柄（約4000）を巡回すると理論上 13 時間超になる。**日付単位の一括取得 API（全銘柄 × 1 日）で日数ループする方式**が取れるか実機確認し、所要時間を見積もること（[jquants.md](jquants.md)）。
>
