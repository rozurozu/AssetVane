...
- ⚠️ **本番投入の実走で判明（2026-06-04・Phase 1 全銘柄バックフィル）**:
  - **スロットルは 16 秒以上にする**。13 秒（4.6 req/分）は 60 秒の窓境界で 5 req に達し、4 営業日処理後に約 5 分ブロックを誘発して `fetch_quotes` が死んだ。**16 秒なら任意の 60 秒窓で最大 4 req** に収まり確実に下回る（`settings.jquants_min_interval_seconds`・既定 16.0）。
