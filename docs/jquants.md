...
- 対策: 株価四本値 API は**日付指定で「その日の全銘柄」を一括取得できる**。✅ **実機確認済み（2026-06）**: `/v2/equities/bars/daily?date=20251215` に `code` を渡さず叩くと、その日の東証全銘柄 **4428 行（1 銘柄 1 行・ETF/REIT 含む）** が 1 リクエスト（＋ページング）で返った。これを使い「日付ループ（営業日 × 1 リクエスト）」でバックフィルすれば、**全銘柄でも 2 年で約 500 リクエスト ＝ Free 5 req/分でも約 100 分**に激減する（銘柄ループの 13 時間超に対し）。実装は `JQuantsAdapter.fetch_daily_quotes_by_date(date)`（Phase 1 の初回バックフィルの入口）。
