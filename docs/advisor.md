...
> **経験レビュー面（reviewer・夜専用・[ADR-081](decisions.md)）**: LLM の面（`FACES`）は chat/nightly/dossier/tagger/triage の **5 面＋ `reviewer` の 6 面**。reviewer は夜バッチ `distill_experience` だけが使う「昨日を振り返る」面で、市場結果で採点した成績（`get_track_record`・[ADR-077](decisions.md)）を教材に、**次に効く教訓だけを知識カードの draft に蒸留**する（自己改善ループ ④「知識へ蒸留」）。他面と違い**最小 toolset**（`get_track_record`/`search_judgments`/`search_cards`/`propose_card`/`adjust_card_weight`）しか見せず（`run_turn(tool_names=…)` の allowlist）、末尾で `persist_card_ops` だけ実行する（多重防御）。起票は常に draft・source は決定論で `'reviewer'`・活性化は人間（[ADR-009](decisions.md)）。活動量ゲート（新しく確定した採点が閾値ぶん溜まった晩だけ発火）＋過学習足切り（十分なサンプルの傾向だけ一般化）で乱発を抑える。provider/model は他面と同じく `/settings` で割り当てる（[ADR-058](decisions.md)・未設定なら沈黙 skip）。
