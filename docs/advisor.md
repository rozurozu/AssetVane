...
| `get_valuation(code)` | バリュエーション/ファンダ事実（PER/PBR/ROE/利益率/配当利回り/YoY 成長率＋業種内ランク＋会社予想の達成率〔`op/profit_forecast_achievement`＝beat/miss〕・上方/下方修正〔`op/profit_forecast_revision`〕＋`last_restatement_at`〔最新の訂正有報・提出日／無ければ null〕・market:JP 明示・verdict なし＝[ADR-048](decisions.md)/[ADR-063](decisions.md) #4）| 2〜 |
...
| `search_cards(query, level?, limit?)` | **知識ベース（知識カード）を意味検索**（[ADR-062](decisions.md)）。市場文脈・手法の解釈・登録した外部知識から今の話題に関係する**非自明な知識**を引く。常時注入の always_inject 以外の具体（銘柄/セクター）知識を掘るときに使う。embedding 未設定時は空＋理由 | 4〜 |
| `propose_card(body, title?, ...)` | 会話から得た**非自明な知識**を知識カードとして**承認制で起票**（[ADR-062](decisions.md) 追補）。後で再利用したい知識を残すときだけ呼ぶ。起票は draft で人間が `/cards` で active 化する | 4〜 |
...
| `get_us_valuation(symbol)` | 米株のバリュエーション/ファンダ事実（`market:US`/`currency:USD` 明示・verdict なし＝[ADR-055](decisions.md)）| 7〜 |
