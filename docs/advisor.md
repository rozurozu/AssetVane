...
| `simulate_trade_impact(action, code, amount_jpy)` | 仮の買い/売りの pro-forma 影響（集中度・相関・vol・リスク寄与・[ADR-085](decisions.md)）| 2〜 |
| `get_position_reviews(portfolio_id)` | 保有の前提崩れ監視（含み損・悪材料・会社予想の未達/下方修正・訂正報告などの材料＋記録済み thesis〔確信度/invalidation/catalyst〕を突き合わせ、見直しが要る保有だけを返す・決定論・JP のみ・[ADR-088](decisions.md)）| 2〜 |
| `get_financials(code)` | 決算数値（生の財務時系列）| 2〜 |
| `get_valuation(code)` | バリュエーション/ファンダ事実（PER/PBR/ROE/利益率/配当利回り/YoY 成長率＋業種内ランク＋会社予想の達成率〔`op/profit_forecast_achievement`＝beat/miss〕・上方/下方修正〔`op/profit_forecast_revision`〕＋`last_restatement_at`〔最新の訂正有報・提出日／無ければ null〕＋売掛/在庫の質〔`receivables/inventory_turnover_days`＝DSO/DIO・`receivables/inventory_growth_yoy`＝[ADR-064](decisions.md) #2・edinetdb.jp 源・null あり〕＋清原式ネットキャッシュ〔`net_cash`＝絶対額・`net_cash_ratio`＝÷時価総額の read-time 導出・[ADR-079](decisions.md)〕・market:JP 明示・verdict なし＝[ADR-048](decisions.md)/[ADR-063](decisions.md) #4）| 2〜 |
| `screen_valuation(criteria)` | バリュエーション条件で割安・優良候補を絞る（しきい値は AI がカード作法で渡す・[ADR-048](decisions.md)）。清原式ネットキャッシュ割安小型株は `net_cash_ratio_min`≥1・`market_cap_max`・`per_max` で（作法は `get_method_card('net_cash_value')`・[ADR-079](decisions.md)）| 2〜 |
...
| `get_method_card(signal_type?)` | **手法の正典的な解釈**（何を測る・スコアの読み方・限界）をリポジトリの手法カードから取得（[ADR-075](decisions.md)/[ADR-079](decisions.md)）。signal 種（lead_lag/ai_alpha/stealth_accum 等）は解釈・引用する前に、strategy 種（清原式 `net_cash_value` 等・signal を持たない screen 手法）は能動 screen の前に呼ぶ（GC/RSI 等の教科書手法は薄い）。`signal_type`（or strategy スラッグ）省略で一覧。カタログは Tool description に常時露出＝skill 型 progressive disclosure（各手法の `native_horizon`＝時間軸も併記し、相談の時間軸に手法を合わせる＝[ADR-091](decisions.md)）| 1〜 |
...
| `propose_trade(action, code, reason, conviction?, invalidation?, catalyst?)` | ニュース起点の買い/売りアイデアを **`proposals`（`kind=buy/sell`）へ承認制で起票**（提示専用・[ADR-052](decisions.md)/[ADR-009](decisions.md)）。`action`=`buy`/`sell`、`code`=JP 5 桁または US ティッカー、`reason`=根拠。分かれば **`conviction`（high/medium/low）・`invalidation`・`catalyst`** も渡し、要素⑤の判断を `proposals.body` に構造化永続する（[ADR-084](decisions.md)・conviction は persist が正規化・不明は落として提案は温存）。**株数・金額などの数値は出さない**（[ADR-014](decisions.md)）。検証 only（実起票は橋渡しが tool_runs から拾う）で、銘柄解決して `{ok, company_name, market}` を返す。未知コードは `{error}`。承認しても発注はしない | 4〜 |
| `propose_watchlist(candidates=[{code, reason}])` | 厳選したウォッチ候補をユーザーに提示（提示専用・[ADR-080](decisions.md)）。`code`=JP 5 桁、`reason`=短い理由（任意）。**watchlist には追加しない**＝候補を `ChatResponse.watchlist_candidates` に載せ、UI がチェックリストで見せてユーザーが選んで `POST /watchlist` する（追加は UI 側＝AI は書かない・[ADR-009](decisions.md)）。検証 only で `{ok, resolved, dropped}` を返す（社名は backend が解決・未知/US は drop＝[ADR-018](decisions.md)）。surfacing は昼 `/chat` だけで**夜 nightly は no-op**（構造で「夜に止まる／勝手に追加」を防ぐ） | 4〜 |
...
> **売買アイデアの起票（[ADR-052](decisions.md)）**: `get_news_context`/`search_news` で根拠（3 層文脈）を掴んだうえで、**強い買い/売り材料があるときだけ** `propose_trade` で起票する（無ければ呼ばない＝毎回出さない）。提示専用ゆえ**株数・金額・目標価格は出さない**（サイズは別途 `optimize_portfolio` の事実計算に委ねる＝[ADR-014](decisions.md)）。方向・根拠に加え、分かれば**確信度・invalidation・catalyst**を構造化して渡し `proposals.body` に永続する（要素⑤の判断を散文に埋もれさせず、採点ループが確信度キャリブレーションを組めるようにする＝[ADR-084](decisions.md)）。夜AI（軸1）・チャット（軸2）の両方から起票でき、起票された提案は `/proposals` 画面で承認/却下する（承認しても発注はしない＝約定後に手入力）。同一 `(kind, code)` の pending は重複起票しない（buy が pending でも同銘柄の sell は起票できる。reject/approve 後は再提案可＝[ADR-052](decisions.md)）。

> **ウォッチ候補の提示（[ADR-080](decisions.md)）**: screen や相談の結果、ウォッチリスト入りが妥当な銘柄群を「本命／次点／…」のように**厳選ショートリストで提示するとき**だけ `propose_watchlist` を呼ぶ（雑談で銘柄名を挙げるだけ・単一銘柄の言及では呼ばない＝乱発しない）。**この Tool は watchlist に追加しない**＝候補を UI にチェックリストで出すだけで、実際に入れるかはユーザーが画面で選ぶ（AI は書かない・承認制＝[ADR-009](decisions.md)）。候補は `ChatResponse.watchlist_candidates`（社名は backend 解決・未知/US は drop）で返り、フローティングと `/advisor` の両方で同じチェックリストが出る。追加は既存 `POST /watchlist`（冪等・reason は note に焼く）。surfacing は昼 `/chat` だけに配線するので、夜 nightly が呼んでも返り値を捨てる no-op＝「夜に選択待ちで止まる／勝手に追加」は原理的に起きない。
...
①コード化済みの手法と、その**手法カード**（③参照知識・[ADR-075](decisions.md)/[ADR-079](decisions.md)）の対応。手法カードは `backend/app/advisor/method_cards/<key>.md`（リポジトリ所有・アプリ非編集）で、`get_method_card(key)` でオンデマンド注入する（skill 型 progressive disclosure）。`kind`＝`signal`（毎晩 signals に焼く・ファイル名＝signal_type）／`strategy`（signal を持たない screen 手法・ファイル名＝スラッグ・[ADR-079](decisions.md)）。

| 手法 | 実装（①コード）| kind／対応 | 投入 | 手法カード（③・method_cards/）|
|---|---|---|---|---|
| モメンタム | `quant/momentum.py` | signal `momentum` | 1〜 | `momentum.md`（薄・教科書）|
| 出来高急増 | `quant/volume_spike.py` | signal `volume_spike` | 1〜 | `volume_spike.md`（薄・教科書）|
| ステルス仕込み | `quant/stealth_accumulation.py` | signal `stealth_accum` | 1〜 | `stealth_accum.md`（厚・独自＝[ADR-074](decisions.md)）|
| AI Alpha Scorer | `quant/ml/`（`infer.py`）| signal `ai_alpha` | 5〜 | `ai_alpha.md`（厚・独自）|
| **業種リードラグ PCA** | `quant/lead_lag.py`（`compute_lead_lag`／`validate_lead_lag`）| signal `lead_lag` | 7〜 | `lead_lag.md`（厚・独自）＋深掘り [docs/methods/lead-lag.md](methods/lead-lag.md)|
| **清原式ネットキャッシュ** | `quant/valuation.py`（`net_cash`）＋`screen_valuation` | **strategy**（signal なし）| 2〜 | `net_cash_value.md`（厚・独自＝[ADR-079](decisions.md)）|

> **[ADR-075/079] 手法カードはリポジトリ所有の第 4 知識源**（CORE/POLICY/knowledge_cards に続く）。手法解釈は knowledge_cards（DB・UI・AI）でなく method_cards（git・code review）が正典で持つ＝手法追加はコード変更を伴うのでアプリからは足さない。**kind で signal（signals に焼く）と strategy（signal を持たない screen 手法・[ADR-079](decisions.md)）の 2 種**。注入は決定論でなく `get_method_card` のオンデマンド（メタ＝summary は Tool description に常時露出・本文は呼んだ時だけ）。教科書手法は薄く、独自手法（lead_lag/ai_alpha/stealth_accum/net_cash_value）は「限界・注意」を厚く。各カードは frontmatter に `native_horizon`（short/medium/long/day＝手法のエッジが実る時間軸）を宣言し、`get_method_card` のカタログに露出させて advisor が相談の時間軸に手法を合わせる（[ADR-091](decisions.md)）。ドリフト検査（orphan/missing）は signal 種のみ対象（strategy は signal に紐づかず対象外）。`docs/methods/*.md` は開発者向け深掘りとして残し、カードから参照。作法は skill `method-card-authoring`。`knowledge_cards.linked_signal_type` は非推奨化し `0035` で DROP 済み（手法↔signal は method_cards がファイル名キーで持つ）。
>
> 業種リードラグ PCA は、米国業種 ETF の当日ショックを事前部分空間へ正則化した PCA で翌営業日の日本業種スコアに写す低ランク予測器（[ADR-039](decisions.md)）。AI は `get_lead_lag()` Tool で**算出済みの事実**（ランキング＋IC/的中率）だけを受け取り、解釈・提示に使う（数値は LLM が作らない＝[ADR-014](decisions.md)）。提示専用（[ADR-009](decisions.md)）。
...
[POLICY]        ← policy をコンパイル（可変・規範＝どうすべきか）
[投資家プロファイル] ← 記述＝行動の癖（鏡・反追従・空なら挿さない・[ADR-082](decisions.md)）
[手法カード]     ← 該当時（手動 or RAG）
...
> **投資家プロファイル（[ADR-082](decisions.md)）**: POLICY（規範）に続く**記述の第 3 層**（この投資家はどういう人か＝行動の癖）。軸1/軸2 の両方に注入し、AI は癖を**打ち消す方向**にコーチングする（鏡・反追従＝迎合しない・使い方の規律は CORE 要素③）。本文は夜バッチ **profiler 面**（7 面目）が取引台帳の行動信号（手仕舞いの帰結・ディスポジション・関心集中）から傾向メモを承認制で蒸留し、人間が `/profile` で承認して育てる（reviewer 面と同型の三層＝job→面→services・活動量ゲート・toolset 制限）。蒸留 Tool `propose_profile_note` は `allowlist_only` で profiler にだけ露出する（chat/nightly には見せない）。`knowledge_cards` とは別物（アイデンティティを市場知識の RAG に混ぜない）。
...
> **経験レビュー面（reviewer・夜専用・[ADR-081](decisions.md)）**: LLM の面（`FACES`）は chat/nightly/dossier/tagger/triage の **5 面＋ `reviewer` の 6 面**。reviewer は夜バッチ `distill_experience` だけが使う「昨日を振り返る」面で、市場結果で採点した成績（`get_track_record`・確信度キャリブレーション／ホライズンキャリブレーション込み＝[ADR-077](decisions.md)/[ADR-084](decisions.md)/[ADR-091](decisions.md)）を教材に、**次に効く教訓だけを知識カードの draft に蒸留**する（自己改善ループ ④「知識へ蒸留」）。他面と違い**最小 toolset**（`get_track_record`/`search_judgments`/`search_cards`/`propose_card`/`adjust_card_weight`）しか見せず（`run_turn(tool_names=…)` の allowlist）、末尾で `persist_card_ops` だけ実行する（多重防御）。起票は常に draft・source は決定論で `'reviewer'`・活性化は人間（[ADR-009](decisions.md)）。活動量ゲート（新しく確定した採点が閾値ぶん溜まった晩だけ発火）＋過学習足切り（十分なサンプルの傾向だけ一般化）で乱発を抑える。provider/model は他面と同じく `/settings` で割り当てる（[ADR-058](decisions.md)・未設定なら沈黙 skip）。

> **提案前の反証面（skeptic・夜専用・[ADR-086](decisions.md)）**: `FACES` の 8 面目。夜バッチ `red_team_proposals`（`distill_investor_profile` の後・通知系の前）だけが使う「独立した反証官（red-team）」で、当夜 pending の buy/sell 提案（軸1 生成／軸2 起票）を**生成面とは別の面**で反証する（真の red-team＝別 model 推奨）。最小 toolset（read-only pull＋提出口 `submit_refutation`〔`allowlist_only`〕）だけ見せ、末尾で `persist_skeptic_reviews_from_tool_runs` が `submit_refutation` だけ拾い、`allowed_ids`（当夜の pending 未反証）に束縛して `proposals.body.skeptic`（`{verdict, refutation, reviewed_at}`）へ注記する。**自動却下はしない**＝status は変えず、`/proposals` の `ProposalCard` に反証を表示して人間が承認判断する（[ADR-009](decisions.md)）。カーソルは持たず `body.skeptic` の有無で有界化（冪等）。`get_signals` の `ai_alpha` を反証材料に引き、ML と thesis のズレを論点化する（[ADR-066](decisions.md)）。障害設計・provider/model 割当は reviewer 同型（未設定は沈黙 skip）。

### 7.1 判断軌跡の観測層（advisor_turns・[ADR-092](decisions.md)）

`get_track_record`（[ADR-077](decisions.md)）が採点する「**結果の質**」に対し、**「AI が実際にどう判断したか（プロセスの質）」を残す層**が `advisor_turns` 台帳。Tool ループ（`run_tool_loop`）を通る **5 面**（chat/nightly/reviewer/profiler/skeptic）の 1 ターンを 1 行に焼く（`generate_once` の dossier/tagger/triage は対象外）。フック `_record_turn` は `run_tool_loop` の単一出口で 1 回だけ呼ばれ、`llm._record_usage` と同型の best-effort（独立接続・失敗は握って応答を壊さない）。**reply 本文は残さず** `tool_sequence`（呼んだ Tool 名＋引数・結果値なし＝[ADR-025](decisions.md)/[ADR-029](decisions.md)）だけ焼き、集計に効く規律 2 列（`called_propose_trade`/`propose_trade_disciplined`）だけ非正規化する。`GET /advisor/turns` が面別サマリ（平均ラウンド・打ち切り率・起票ターン数・規律充足率）＋直近の軌跡を返し、frontend `/advisor-turns` で「6 ラウンド上限で分析が打ち切られていないか」「conviction/invalidation/catalyst/horizon が実際に埋まっているか」「どの面が空回りしているか」を数字で見る（[data-model.md](data-model.md)）。

> **夜AI 単体起動口（[ADR-092](decisions.md)）**: 全 NIGHTLY（重い全銘柄バックフィル含む）を回す `POST /batch/run` と別に、advisor 部分（`run_advisor` ジョブ 1 本）だけを軽く回す `POST /batch/run-advisor`（[ADR-011](decisions.md)「1つの脳・2つの起動口」の 3 例目・`/settings` の「夜AI を今すぐ回す」）。fetch/score は混ぜない（事実の鮮度は事前の `POST /batch/run` で担保）。nightly 面が未設定なら 400 で /settings 誘導。何度でも叩いて `/advisor-turns` で判断軌跡を観測する。
