...
| `get_method_card(signal_type?)` | **手法の正典的な解釈**（何を測る・スコアの読み方・限界）をリポジトリの手法カードから取得（[ADR-075](decisions.md)/[ADR-079](decisions.md)）。signal 種（lead_lag/ai_alpha/stealth_accum 等）は解釈・引用する前に、strategy 種（清原式 `net_cash_value` 等・signal を持たない screen 手法）は能動 screen の前に呼ぶ（GC/RSI 等の教科書手法は薄い）。`signal_type`（or strategy スラッグ）省略で一覧。カタログは Tool description に常時露出＝skill 型 progressive disclosure | 1〜 |
...
> **経験レビュー面（reviewer・夜専用・[ADR-081](decisions.md)）**: LLM の面（`FACES`）は chat/nightly/dossier/tagger/triage の **5 面＋ `reviewer` の 6 面**。reviewer は夜バッチ `distill_experience` だけが使う「昨日を振り返る」面で、市場結果で採点した成績（`get_track_record`・確信度キャリブレーション込み＝[ADR-077](decisions.md)/[ADR-084](decisions.md)）を教材に、**次に効く教訓だけを知識カードの draft に蒸留**する（自己改善ループ ④「知識へ蒸留」）。他面と違い**最小 toolset**（`get_track_record`/`search_judgments`/`search_cards`/`propose_card`/`adjust_card_weight`）しか見せず（`run_turn(tool_names=…)` の allowlist）、末尾で `persist_card_ops` だけ実行する（多重防御）。起票は常に draft・source は決定論で `'reviewer'`・活性化は人間（[ADR-009](decisions.md)）。活動量ゲート（新しく確定した採点が閾値ぶん溜まった晩だけ発火）＋過学習足切り（十分なサンプルの傾向だけ一般化）で乱発を抑える。provider/model は他面と同じく `/settings` で割り当てる（[ADR-058](decisions.md)・未設定なら沈黙 skip）。
