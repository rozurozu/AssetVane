...
| `propose_card(body, title?, ...)` | 会話から得た**非自明な知識**を知識カードとして**承認制で起票**（[ADR-062](decisions.md) 追補）。後で再利用したい知識を残すときだけ呼ぶ。起票は draft で人間が `/cards` で active 化する。**壁打ちの作法**＝CORE 要素④の規律で「合意してから起票」（[ADR-065](decisions.md)）。起票した draft の id は `ChatResponse.card_ids` で frontend に返り、チャット内に `/cards` 導線をインライン表示する | 4〜 |
