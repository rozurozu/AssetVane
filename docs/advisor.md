...
| `search_cards(query, level?, limit?)` | **知識ベース（知識カード）を意味検索**（[ADR-062](decisions.md)）。市場文脈・手法の解釈・登録した外部知識から今の話題に関係する**非自明な知識**を引く。常時注入の ambient 以外の具体（銘柄/セクター）知識を掘るときに使う。embedding 未設定時は空＋理由 | 4〜 |
