...
| `get_method_card(signal_type?)` | **手法の正典的な解釈**（何を測る・スコアの読み方・限界）をリポジトリの手法カードから取得（[ADR-075](decisions.md)）。独自手法（lead_lag/ai_alpha/stealth_accum 等）を解釈・引用する前に呼ぶ（GC/RSI 等の教科書手法は薄い）。`signal_type` 省略で一覧。カタログは Tool description に常時露出＝skill 型 progressive disclosure | 1〜 |
...
①コード化済みの手法と、その**手法カード**（③参照知識・[ADR-075](decisions.md)）の対応。手法カードは `backend/app/advisor/method_cards/<signal_type>.md`（リポジトリ所有・アプリ非編集）で、`get_method_card(signal_type)` でオンデマンド注入する（skill 型 progressive disclosure）。

| 手法 | 実装（①コード）| 対応 signal | 投入 | 手法カード（③・method_cards/）|
|---|---|---|---|---|
| モメンタム | `quant/momentum.py` | `momentum` | 1〜 | `momentum.md`（薄・教科書）|
| 出来高急増 | `quant/volume_spike.py` | `volume_spike` | 1〜 | `volume_spike.md`（薄・教科書）|
| ステルス仕込み | `quant/stealth_accumulation.py` | `stealth_accum` | 1〜 | `stealth_accum.md`（厚・独自＝[ADR-074](decisions.md)）|
| AI Alpha Scorer | `quant/ml/`（`infer.py`）| `ai_alpha` | 5〜 | `ai_alpha.md`（厚・独自）|
| **業種リードラグ PCA** | `quant/lead_lag.py`（`compute_lead_lag`／`validate_lead_lag`）| `lead_lag` | 7〜 | `lead_lag.md`（厚・独自）＋深掘り [docs/methods/lead-lag.md](methods/lead-lag.md)|

> **[ADR-075] 手法カードはリポジトリ所有の第 4 知識源**（CORE/POLICY/knowledge_cards に続く）。手法解釈は knowledge_cards（DB・UI・AI）でなく method_cards（git・code review）が正典で持つ＝手法追加はコード変更を伴うのでアプリからは足さない。注入は決定論でなく `get_method_card` のオンデマンド（メタ＝summary は Tool description に常時露出・本文は呼んだ時だけ）。教科書手法は薄く、独自手法（lead_lag/ai_alpha/stealth_accum）は「限界・注意」を厚く。`docs/methods/*.md` は開発者向け深掘りとして残し、カードから参照。作法は skill `method-card-authoring`。`knowledge_cards.linked_signal_type` は本 ADR で非推奨化し `0035` で DROP 済み（手法↔signal は method_cards がファイル名キーで持つ）。
>
