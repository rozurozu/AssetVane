...
5. **`backend/.env`（秘密）を用意する**。初回はラズパイに存在しないので自分で置く（J-Quants/LLM/Discord キー・リポジトリには入れない＝ADR-005）。どちらかで:
   ...
   > ⚠️ **`CORS_ALLOW_ORIGINS` を本番のオリジンに合わせること**（既定は dev 用の
   > `http://localhost:3000`）。ダッシュボードを `http://192.168.x.x:3000` で開くなら
   > `CORS_ALLOW_ORIGINS=http://192.168.x.x:3000`、`raspberrypi.local` で開くなら
   > `http://raspberrypi.local:3000`。**「ブラウザで開く URL のホスト」「frontend に焼く `API_URL`
   > のホスト」「この CORS のホスト」の 3 つが一致していないと、画面に「backend 未接続」バッジが
   > 出る**（`/health` の fetch が CORS で弾かれる）。詳細は下の「トラブルシュート」。
   > 変更後は `make reload`（コンテナ再生成で `.env` を読み直す）で反映する。`make restart` は
   > 停止→起動のみで `.env` を読み直さないので注意。
...
- **画面に「backend 未接続」バッジが出る**: バッジはブラウザから `${API_BASE}/health` への fetch が失敗すると出る（`Topbar`）。デプロイの `/health` チェックは **Pi 内の `localhost:8000`** を見るだけなので、ここが通っても**ブラウザ経路は別問題**。原因は次の **3 つのホストの不一致**がほとんど（ブラウザ DevTools の Network で `/health` の Request URL と Console の CORS エラーを見れば切り分く）:
  1. **ダッシュボードを開く URL のホスト**（＝ブラウザの Origin。`http://192.168.x.x:3000` か `http://raspberrypi.local:3000` か）
  2. **frontend に焼かれた `API_URL` のホスト**（既定 `http://raspberrypi.local:8000`。`deploy.env` の `API_URL` で決まる）
  3. **backend の `CORS_ALLOW_ORIGINS` のホスト**（`backend/.env`。既定 `http://localhost:3000`）

  この 3 つが**同じホスト**を指していないと、(2) に届かない（DNS/接続エラー）か (3) で弾かれる（CORS エラー）かのどちらかで fetch が throw する。直し方:
  - **CORS だけのズレ**（Console に "blocked by CORS policy"）: `backend/.env` の `CORS_ALLOW_ORIGINS` を (1) に合わせて `make reload`（コンテナ再生成で `.env` 反映・再ビルド不要。`make restart` は読み直さないので不可）。
  - **焼き込み URL のズレ**（`ERR_NAME_NOT_RESOLVED` / `ERR_CONNECTION_REFUSED`）: `deploy.env` の `API_URL` を (1) のホストに合わせて `make deploy`（**再ビルドが要る**）。
  - DHCP で IP が変わると再発するので、IP で運用するなら DHCP 予約/固定 IP に、引けるなら 3 つとも `raspberrypi.local` で統一すると安定。
- **arm64 になっているか確認**:
  ```bash
