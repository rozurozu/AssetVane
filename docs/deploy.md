...
6. **（任意）AI を codex で動かすなら `auth.json` を置く**（[ADR-032](decisions.md)）。codex CLI は backend イメージに同梱済み（Node 不要の musl バイナリ・rust-v0.137.0）なので**ラズパイに codex を入れる必要はない**。必要なのは login 済みの `auth.json` だけ。codex login は ChatGPT のブラウザ認証が要るので、**ヘッドレスなラズパイでは行わず開発機（Mac）で済ませて運ぶ**:
   ...
   > ⚠️ **LLM の初回設定（[ADR-058](decisions.md)）**: provider/api_key/model は env ではなく DB に保存し `/settings` で登録する。**migration（`alembic upgrade head`／起動時 `init_db`）後、`/settings` で provider を登録するまで鍵あり面（chat/nightly/dossier/tagger/triage）の LLM は動かない**（chat は 503・夜間/ドシエは通知付き skip・タグ付け/カード審査は沈黙 skip）。codex は鍵なし組み込みなので面に割り当てれば最初から使える。
