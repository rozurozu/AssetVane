...
6. **LLM の初回設定（[ADR-058](decisions.md)）**: provider/api_key/model は env ではなく DB に保存し `/settings` で登録する。**migration（`alembic upgrade head`／起動時 `init_db`）後、`/settings` で provider を登録するまで LLM 面（chat/nightly/dossier/tagger/triage/reviewer）は動かない**（chat は 503・夜間/ドシエは通知付き skip・タグ付け/カード審査/経験蒸留は沈黙 skip）。シードしないので初回は手動登録が要る（provider は OpenAI 互換のみ・codex 経路は [ADR-073](decisions.md) で撤去）。
