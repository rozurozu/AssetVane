...
6. **（任意）AI を codex で動かすなら `auth.json` を置く**（[ADR-032](decisions.md)）。codex CLI は backend イメージに同梱済み（Node 不要の musl バイナリ・rust-v0.137.0）なので**ラズパイに codex を入れる必要はない**。必要なのは login 済みの `auth.json` だけ。codex login は ChatGPT のブラウザ認証が要るので、**ヘッドレスなラズパイでは行わず開発機（Mac）で済ませて運ぶ**:
   ...
   ```
   その後 `backend/.env` に `LLM_PROVIDER_CHAT=codex`（必要なら `CODEX_MODEL=gpt-5.5`）を入れて `make reload`。`compose.prod.yaml` が `./.codex` を `/root/.codex` に **read-write** マウントする。
   > ⚠️ `auth.json` はトークンリフレッシュで **codex 自身が書き換える**（だから read-write）。毎晩 nightly/chat で codex を動かしていればリフレッシュされ続け実用上は失効しにくいが、**長期放置すると失効**する。失効すると codex 呼び出しが失敗して [ADR-018](decisions.md) で Discord 通知に出るので、気づいたら開発機で `codex login` し直して再コピーする。無人 cron での継続が未実証のため、`LLM_PROVIDER_NIGHTLY` は当面 `openai` 推奨（[ADR-032](decisions.md)）。
