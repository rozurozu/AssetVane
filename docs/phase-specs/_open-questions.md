...
- **検証メモ（裁定不要・2026-06-04）: Phase 3 完了条件＝機構は検証済み／LLM 通しは未検証。**
  承認制 policy 更新の**機構**（`submit_journal` → proposal 起票 → 承認 → `apply_policy_change` で policy 反映）は引数を直接与えて決定論的に通ることを確認済み（健全）。一方、完了条件の通しパス（チャット相談 → AI が `submit_journal` を中身付きで呼び policy 提案／夜の分析AI が翌朝それを出す）は、ローカル 9B（`qwen3.5:9b`）で chat 1 回・nightly 1 回試したところ、どちらも read 系 Tool を 1 回呼んだ後に空応答で `submit_journal` 不呼び出し（journal は空 1 件・proposal 起票なし）だった。ただし **N=2 の非決定サンプルなので「弱モデルでは無理」とは断定できない**（リトライ・プロンプト・`tool_choice` 等の条件を潰していない）。これは **「ブロッカー」ではない**。
  **裁定（2026-06-05）**: 本番は強モデル前提（[ADR-012](../decisions.md)）。ローカル 9B（`qwen3.5:9b`）は **開発時の動作確認専用**で、`submit_journal` を確実に呼ばせる作り込み（指示文強化・空応答リトライ・`tool_choice` 強制・再現確認）は **しない＝弱モデルでできないことは割り切る**。機構（引数を直接与えた決定論パス）は検証済み・健全。残るのは LLM **障害時フォールバックの堅牢化**（[ADR-018](../decisions.md)）であり、これはモデル品質と独立の無人運用要件。
