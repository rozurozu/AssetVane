...
>
> **注目シグナル リデザインの追補（2026-06-30 設計確定・[ADR-067](../decisions.md)・実装は後続）**: 「注目シグナル」を **score 閾値 Top N 抽出**から **合流(confluence)ゲート＋AI 選別**へ作り直す（実機で注目 1294/2642 件＝`golden_cross` 単独 notable と score 1.00 飽和が発生源。score 降順 Top10 では希少イベントが 1.00 の山に埋もれ既に表示できていなかった）。**§3 の `_is_alert`／score≥`ALERT_SCORE_MIN` の Top N 抽出はこの ADR で上書き・撤去**（`ALERT_SCORE_MIN`/`ALERT_TOP_N`/`list_signals_for_alert` の Top N 用途は廃。`quant` の score 勾配自体は [ADR-026](../decisions.md) 資産で温存）。
> - **候補集合（Python が決定論的に構築）**: 独立した**材料次元**を判定し、広い母集団は**材料2次元以上が重なった銘柄だけ**候補にする（GC 単独・トレンド継続単独は落ち `/signals` の DB 記録にのみ残す＝安全網）。材料次元（独立4つ・相関は1つに数える）＝①値動き（当日大幅変動 ±X% or GC or RSI 反転）／②出来高急増（volume_spike ratio 高）／③ニュース（直近 24h polarity 付き stock 層）／④リードラグ（当日 lead_lag リーダー）。ai_alpha は `.pkl` 配置後に⑤として自動参入。**carve-out**＝出来高極増（ratio≥高閾値）は単独でも候補／レーダー枠（保有∪ウォッチリスト）は材料1次元で候補（材料ゼロは黙る）。各候補に「点いた材料＋値」のタグを付ける（[ADR-014](../decisions.md)＝Python は事実）。
> - **連携**: 候補を**夜AI プロンプトに直接注入**（堅牢・ツール依存なし）＋ `get_notable_candidates` Tool でも公開し昼チャット救済。AI は **新 Tool `submit_notable_stocks(picks=[{code, reason}])`** で選別結果を返し、`persist_notable_picks_from_tool_runs`（begin() 内）が **新テーブル `notable_picks`** へ永続（migration `0032`）。digest が後で読む。
> - **digest 本文**＝【決定論: 保有の悪材料（上記 ADR-051 追補②を維持＝AI 拾い忘れの安全装置）】＋【AI 選別の注目（`notable_picks`）】＋【極薄サマリ 1 行（signals N／イベント M／候補 K／AI 選別 P・ドロップ件数）】。①急騰落の自動説明（ニュース attach）は AI の reason へ統合（候補タグにニュースを含むため）。
> - **つまみ**＝手法閾値（大幅変動 %・出来高極増 ratio・合流次元数）は新 quant/service の定数、運用つまみ（候補総数上限・digest 表示数）は `config.settings`（[ADR-016](../decisions.md)/[ADR-027](../decisions.md)）。**以降 §1〜§3 の「⑧ アラート条件＝score≥0.6 / Top10」の記述は ADR-067 に従って読むこと**（テーブル `notify_key='signal:*'` 等の歴史記述は据え置き）。
>
