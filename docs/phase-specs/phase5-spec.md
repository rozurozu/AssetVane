...
> 出所: roadmap.md Phase 5 / ADR-006(学習別PC・推論のみ)/ADR-016(手法はコード)。レビュー・裁定反映済み。**現状＝推論経路＋初回学習まで実装済み**（`quant/ml/{features,train,infer}.py`・`ml/model_store.py`・夜間ジョブ `score_ai_alpha`・signals `signal_type=ai_alpha`）。初回学習は Mac コンテナで実測ずみ（[ADR-066](../decisions.md)・`.pkl` 配置＋[ml-training.md](../ml-training.md) の `【実測】` 欄穴埋め済み。Mac 学習に固定せずスペック要時は GPU ゲーミングPC）。以下は着工時の仕様。
>
...
- **ラベル（教師信号・既定）**: **決算開示後 60 営業日の対 TOPIX 超過リターン**（`(銘柄の60営業日後 adj_close リターン) − (TOPIX の同期間リターン)`）を **回帰**で予測（分類より情報量が多い＝quant.md §5.2）。`index_quotes` の TOPIX 終値をベンチに使う。**U-4 裁定済み**（§11）: **60 日回帰で確定**（分類化しない・中期ファンダ枠）。回帰/分類・期間を後から差し替えられる形（train.py 引数・`docs/ml-training.md` 記載値）で実装。
...
- **U-4: P5 ML の予測ラベル** ✅裁定済み — **60 営業日先の対 TOPIX 超過リターンを回帰**で確定。理由: (1) 回帰は大きさの情報を保ち**ランク正規化→ score 0..1** の順位付け用途に向く（分類は情報が痩せる・評価も IC=順位相関）、(2) 特徴量が `financials` 中心の**ファンダ＝動きが遅い**ので 60 日ホライズンと噛み合い、Phase 1 の短期 momentum/volume と**役割が重ならない中期枠**になる。**分類化はしない**。後から `train.py` の `label_kind`/`label_horizon_days` 引数と `docs/ml-training.md` の記載値で差替可（学習やり直しコスト中）。
