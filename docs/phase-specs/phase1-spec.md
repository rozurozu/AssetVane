...
**スコア定義（0.0〜1.0）**（U-1・既定）:
```
score = 0.6 * gc_today + 0.4 * rsi_rev_today
```
両方成立で 1.0／GC のみ 0.6／RSI 反転のみ 0.4／どちらも無ければ `None`（行を作らない）。
...
```json
{
  "date": "2025-12-15", "code": "72030", "signal_type": "momentum", "score": 0.6,
  "payload": {
    "golden_cross": true, "rsi_reversal": false,
...
```json
{
  ...
  "payload": {
    "volume": 4200000.0, "vol_ma20": 1000000.0, "ratio": 4.2, "adj_warning": false,
...
Phase 1 に効くユーザー裁定 3 件。**いずれも推奨値を spec の既定として採用済み**で、後から env/設定で差し替え可能（コード作り直し不要）。詳細・代替案は `docs/phase-specs/_open-questions.md`。

| # | 論点 | 既定値（採用） | 代替 | 差替の容易さ |
|---|---|---|---|---|
| **U-1** | momentum スコアの重み | GC 0.6 / RSI 反転 0.4 | 等加重 / GC のみ / 重み調整 | 易（設定値） |
| **U-2** | volume_spike 閾値・スコア | 当日÷過去20日平均 ≥ **3.0倍**・`score=min(ratio/10, 1.0)` | 閾値 2.0〜5.0・スケール変更 | 易（設定値） |
| **U-9** | 夜間 cron の起動時刻 | **02:00 JST**（TZ=Asia/Tokyo） | 生活時間に合わせ変更 | 易（cron 設定） |
