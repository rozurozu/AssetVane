...
> 採番: `0007_dossier`（down_revision=`0006_advisor_state`）。watchlist/stock_dossiers/dossier_sources の 3 テーブルは **ai-advisor が正本定義**（_arbitration 決定1・B-13）。移行ファイルの発行（作成）は data-arch が代行、DDL 本体は本仕様が正本。
...
## 2. スキーマ変更（`0007_dossier`）

**Alembic**: `revision="0007_dossier"`・`down_revision="0006_advisor_state"`（_arbitration 決定1 の通し番号表）。ファイル発行は data-arch が代行、DDL 正本は本仕様（B-13）。`schema.py` の同一 `metadata` に 3 テーブルを追記（_current-state.md §1 の Table 定義流儀）。**watchlist は Phase 2（旧 data-arch 案）から外し、ここに一本化**（二重 CREATE 防止）。
...
- **既定（L-22 / 決定7 / _open-questions U-8）**: watchlist を `last_investigated_at` が**古い順**に並べ、**1 晩 N 件**だけ `investigate_stock(code, mode="nightly")` を回す。`N` はユーザー指定（設定値・易に変更可）。
- **stale しきい値 = 21 日**（backend 算出）。`last_investigated_at` が 21 日より古い（または未調査）の銘柄を優先的に巡回対象に選ぶ。
- **夜間の起動**: data-arch の `NIGHTLY_JOBS` 末尾付近に巡回ジョブを相乗り（APScheduler 同居・方式C）。cron 起動時刻は **02:00 JST**（U-9・既定）。MCP 非依存で軽く回す（ADR-020）。
- **コスト**: 夜間でも LLM 要約コストが乗る（watchlist N 銘柄 × 毎晩）。N・頻度（毎晩／週次）は `_open-questions.md` U-8 でユーザー確認に回す（既定値で着工可）。
...
```
- `GET /dossiers/{code}` → `Dossier`（未調査なら 404 または `summary_md: ""`＋空 `sources`）。
...
- **REST**: `GET /watchlist`（stale 算出 21 日境界）・`POST/DELETE /watchlist`・`GET /dossiers/{code}`（未調査 404 or 空）・`POST /dossiers/{code}/investigate`（同期で最新ドシエ返却）。
- **migration**: `test_migrations` に `0007_dossier` 適用後 `watchlist`/`stock_dossiers`/`dossier_sources` が存在することを追加。watchlist が二重 CREATE されない（`0004` に無い）こと。
...
1. [ ] `0007_dossier` 移行発行（data-arch 代行）＋ `schema.py` に `watchlist`/`stock_dossiers`/`dossier_sources` を追記（DDL 正本=本仕様）。`test_migrations` に存在確認を追加。
...
| # | 論点 | R3 後の状態 / 既定値 |
|---|---|---|
| **U-8（ユーザー裁定）** | 夜間ドシエの調査頻度・watchlist 上限 N・stale しきい値 | **既定**: `last_investigated_at` が古い順に **1 晩 N 件**（N はユーザー指定）／**stale しきい値 = 21 日**（backend 算出）／夜間 cron **02:00 JST**（U-9・別件）。N・頻度（毎晩／週次）・閾値は `_open-questions.md` U-8 でユーザー確認。値は設定値で易に差し替え可（コード作り直し不要）。 |
