...
```
- `GET /dossiers/{code}` → `Dossier`（**未調査でも 200＋空ドシエ＝`summary_md: ""`＋空 `sources`＋`last_investigated_at: null`**・確定）。当初 spec は「404 または空ドシエ」の二択を残していたが、frontend の `DossierSection` が未調査銘柄でも常に描画し「調査する」ボタンを出す設計（§7・`last_investigated_at` 流儀）と素直に噛み合う **200 固定**で確定した（2026-06-08 判断・404 は採らない）。未調査の区別は `last_investigated_at: null` で表す。
...
- **REST**: `GET /watchlist`（stale 算出 21 日境界）・`POST/DELETE /watchlist`・`GET /dossiers/{code}`（未調査でも **200＋空ドシエ**で確定・§5.2 参照）・`POST /dossiers/{code}/investigate`（同期で最新ドシエ返却）。
