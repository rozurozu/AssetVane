...
- **段階**: 実装（2026-07-02・ATDD）。着手物＝① `advisor/method_cards.py`（frontmatter パーサ＋起動時ロード＋index/get＋ドリフト検査）② `advisor/method_cards/{momentum,volume_spike,stealth_accum,lead_lag,ai_alpha}.md` ③ Tool `get_method_card`（registry/schemas/handlers・description に動的カタログ）④ `card_triage` の linked_signal_type 出力停止＋schema コメント deprecated ⑤ テスト（loader/tool/ドリフト）⑥ skill `method-card-authoring` 新設 ⑦ docs 同期（本 ADR・advisor.md・CLAUDE.md・data-model.md〔linked_signal_type deprecated〕・ADR-074/062 に注記）。backend pytest／ruff／pyright green。**LLM が実際に get_method_card を呼んで解釈に使うかは次回運用時に確認**（本 ADR 起票時は migration 無し）。**追記（2026-07-02）: 先送りしていた `linked_signal_type` 列の DROP を別 PR で完了（migration 0035・SQLite は batch_alter_table）。schema/repo/services/router/card_triage の残配線と、連鎖して浮いていた `card_triage.assist_card` の `existing_signal_types` 引数＋router の `_IMPLEMENTED_SIGNAL_TYPES` 定数（linked_signal_type 生成停止で未使用化）、frontend の型/表示も同 PR で撤去。値は破棄で可（method_cards が索引を持つ・deprecated 列）。ADR-062 の linked_signal_type 記述は当時の設計記録として残置。**
...
## ADR-076: ドシエ調査の進行状態をプロセスメモリで持ち GET /dossiers/{code} に露出する（リロードで「調査中」を保つ）

- **状況**: 銘柄詳細（`/stocks/[code]`）の `DossierSection` で「調査する」を押すと `POST /dossiers/{code}/investigate` が走り「調査中…（数十秒かかるのだ）」と出る。だが調査中は frontend のローカル `useState(investigating)` **だけ**が保持しており、この最中にリロードすると初期値 `false` に戻り、`GET /dossiers/{code}` の `last_investigated_at=null`（未調査）を見て「調査する」ボタンに戻ってしまう。裏取りの結果、**サーバ側は「今この銘柄を調査中」という事実を一切記録していない**＝DB に status 列も銘柄ロックも無く（`watchlist`/`stock_dossiers` に status 無し・[ADR-020](#adr-020-個別銘柄ドシエ定性ファンダ調査-1銘柄1レポートを更新し続ける) は「1 パイプライン・複数起動口」で銘柄単位の排他も持たない）、POST は完了まで待つ完全同期（数十秒）。よって観測者（リロード後の同タブ・別タブ）が調査中を知る術が無い。
- **決定**:
  - **「今この銘柄を調査中か」を FastAPI プロセスメモリの参照カウント・レジストリ（`app/advisor/dossier_progress.py`）で持つ**。`investigate_stock`（共通パイプライン）の冒頭で `mark(code)`・`finally` で `unmark(code)`。手動 POST・夜間巡回・チャット Tool のどの起動口でも 1 箇所で刻まれる。
  - **`GET /dossiers/{code}` のレスポンス（`Dossier`）に `investigating: bool` を追加**し、`_build_dossier` が `dossier_progress.is_investigating(code)` を載せる。未調査（ドシエ行が無い初回調査中）でも `investigating` は立つ。
  - **frontend はロード時にサーバの `investigating` を読んで「調査中…」表示を復元**し、`true` の間は `getDossier` を数秒間隔でポーリングして完了を検知→ドシエへ差し替える（`DossierSection.tsx`）。ローカルの投稿中フラグと OR して二重起動も防ぐ。
  - **DB には永続しない**（migration 無し）。プロセスが死ねばフラグも消える＝クラッシュ/再起動/`--reload` で「調査中が永遠に残る」ゴミが生まれない（[batch/state.py](#adr-036-バッチは停止できる状態が見える実行状態はメモリ-singleton停止は協調キャンセル)＝ADR-036/070 と同じ論拠）。
  - **参照カウント**にする理由＝同一 code を夜間巡回と手動が同時調査しうる（銘柄ロック無し・ADR-020）。単純な集合だと片方の完了で早期クリアするため、全調査が終わるまで `is_investigating` を `True` に保つ。
  - **POST は同期のまま**（202＋バックグラウンドジョブ化はしない＝Minimal Impact）。
- **理由**: DB に触れる OS プロセスは FastAPI 1 つ（[ADR-005](#adr-005-db-に触れるのは-fastapi-のみnext-は-rest-経由)）ゆえ、調査を走らせる主体とそれを露出する GET が同一プロセスにあり、メモリ・シングルトンで真偽が整合する。永続化（DB 列）は crash 時に stale フラグが残り掃除が要るのに対し、プロセスメモリは死ねば消えるので進行状態には最適（batch state と同型）。`finally` で落とすため、クライアント切断でハンドラが `CancelledError` でキャンセルされても（[ADR-072](#adr-072-ai-advisor-チャットを送信中にキャンセルして編集送り直しできるようにする) の is_disconnected 打ち切りと同系の切断意味論）フラグは正しく落ち、「調査が本当に止まればフラグも消える／続いていれば残る」が自動で整合する。
- **代替案**: **(A) DB に status 列**→ migration＋crash 時の stale 掃除が要り、[ADR-005](#adr-005-db-に触れるのは-fastapi-のみnext-は-rest-経由) の単一プロセス前提では過剰＝メモリで足りる。**(B) sessionStorage 等ブラウザ側のみ**→ backend 無改変だが別タブ/別端末で不可視・失敗/完了の検知を自前で持ち、フラグが実態とズレる（hacky）＝サーバに真実を置く方針に反する。**(C) POST を非同期ジョブ化（202＋poll）**→ 大きな作り替えで `conn` ライフサイクルにも波及＝要望（リロードで調査中表示）に対し過大。
- **段階**: 実装（2026-07-02・ATDD）。着手物＝① `advisor/dossier_progress.py`（参照カウント・`threading.Lock`）② `investigate_stock` を try/finally で mark/unmark ③ `routers/dossier.py` の `Dossier` に `investigating`・`_build_dossier` でセット ④ frontend の `Dossier` 型＋`DossierSection`（`isInvestigating`＋完了ポーリング）⑤ テスト（レジストリの参照カウント回帰・GET 露出）⑥ docs 同期（本 ADR・api.md §5）。backend pytest／ruff／pyright green・frontend Biome green。**リロード（切断）後も調査がサーバで完走し `investigating` が保たれるかは dev サーバでの実機確認に回す**（通常 Starlette は切断でハンドラを自動キャンセルしないので minimal 改修で足りる見込み・万一殺す挙動なら `asyncio.create_task` で request から切り離す追補）。スコープ外＝watchlist 一覧（`/watchlist`）の同型 ephemeral 問題（backend レジストリは共通なので後日 `GET /watchlist` に per-row `investigating` を足せば拡張可）。
- **関連**: [ADR-005](#adr-005-db-に触れるのは-fastapi-のみnext-は-rest-経由)（DB に触るのは FastAPI 1 プロセス＝メモリ真実が整合）・[ADR-020](#adr-020-個別銘柄ドシエ定性ファンダ調査-1銘柄1レポートを更新し続ける)（1 パイプライン・複数起動口・銘柄ロック無し＝参照カウントの根拠）・[ADR-036](#adr-036-バッチは停止できる状態が見える実行状態はメモリ-singleton停止は協調キャンセル)（バッチ実行状態のプロセスメモリ singleton＝同型の前例）・[ADR-072](#adr-072-ai-advisor-チャットを送信中にキャンセルして編集送り直しできるようにする)（切断＝CancelledError の意味論）・[api.md](api.md)（§5 Dossier に investigating）。

## ADR-077: AI の過去提案を市場結果で採点する夜間ループ（提示ベースの銘柄選択スキル評価・テーマ A）

- **状況**: AI Advisor は毎晩 buy/sell 提案（`proposals`・[ADR-052](#adr-052-ニュース起点の売買アイデアを承認制で起票する)）と注目選別（`notable_picks`・[ADR-067](#adr-067-夜-digest-の注目シグナルを合流ゲートai-選別に作り直す)）を出すが、**それらがその後の市場でどうだったかの事後評価が一切無い**。`proposals.outcome` 列は承認/却下の人手メモで市場結果ではなく、backend 全体で市場採点に読む箇所はゼロ。`NIGHTLY_JOBS`（30 本）は全て forward-looking で「昨日を振り返る」ジョブが 1 本も無い。結果、AI は自分の的中率を知らないまま同じ提案を繰り返しうる。裏取りの結果、材料は既に揃っている＝価格は `daily_quotes`/`us_daily_quotes`、ベンチ（TOPIX/S&P500）は `index_quotes` の `^TPX`/`^SPX`（`services/portfolio.py` の既定ベンチ）、営業日カウントの系列方式は `quant/ml/train.py`（対 TOPIX 60 営業日超過）に前例がある。`tasks/hermes-transfer-2026-07-02.md`（hermes-agent の自己改善ループ移植提案）の骨格＝「会話を読み直して学ぶ」を投資では「**過去提案を市場結果で採点して学ぶ**」に差し替える、その最初の一歩（テーマ A）。
- **決定**:
  - **採点母集団 = buy/sell 提案 ＋ notable_picks**（`policy_change`/`rebalance` は銘柄の方向性が無いので対象外）。buy/sell は方向性ありで的中フラグ、notable は非方向でリターン記録のみ（`hit=NULL`）。
  - **起点（entry）= 提案日（`created_date`/`date`）以上の最初の実在バーの adj_close**（forward・休場/未取得なら翌営業日へ前進・データ未達なら pending）。preceding にすると鮮度遅延時に古い終値へ誤アンカーするため forward を採る。
  - **horizon = 20 と 60 営業日の 2 本立て**（`horizon` 列で 1 提案 = 複数行）。short catalyst と medium thesis（ML の対 TOPIX 60 日規約と一致）を両方測る。未経過は `status='pending'` で保留し翌晩以降 UPSERT で確定。
  - **営業日カウントは株価系列そのもの**（N 本先の終値＝到達）で数える（別カレンダー非依存・DB grounded・休場を自然吸収＝`train.py` の系列方式と同型）。
  - **リターンは絶対（`realized_return`）＋対ベンチ超過（`excess_return`・JP=`^TPX`/US=`^SPX`）を両方保存**。ベンチ欠測時は `excess=NULL`＋`benchmark_fallback=1`（絶対で判定した合図）。
  - **hit は超過リターンで判定**（buy→`excess>0`・sell→`excess<0`・notable→NULL）。銘柄選択スキルを市場ベータから分離。ベンチ欠測時は絶対リターンの符号にフォールバック。
  - **新テーブル `proposal_outcomes`（migration `0036`）**＋**純関数 `quant/outcome.py`**（`compute_horizon_outcome`/`classify_hit`・DB/today 非依存・[ADR-014](#adr-014-ai-に数値を計算させないpython-が事実を計算し-ai-は判断する)/[ADR-016](#adr-016-手法はテスト済みコードで実装する-ai-にその場で計算させない)）＋**`services/track_record.py`**（下ごしらえ＋集計）＋**夜間ジョブ `score_proposal_outcomes`**（夜バッチ初の backward-looking・`run_advisor` の後・`notify_digest` の直前・冪等 UPSERT）＋**Tool `get_track_record`**（pull・`min_phase=1`・read-only）。CORE 要素④に「再提案前に `get_track_record` で成績確認」の一文（意図的コミット＝[ADR-015](#adr-015-システムプロンプトは-core可変policy-の-2-層)）。
  - **US buy/sell も採点対象**（`us_daily_quotes`＋`get_us_quotes` が既存・当初案の「US は skip」は不要）。
  - **採点入口の有界化＝final は再採点しない（2026-07-02 追補）**: 採点は `(origin_kind, origin_id, horizon)` 単位で判定し、既 final のキーは毎晩スキップする（1 提案は 20/60 の 2 horizon を持ち「提案から 20〜60 営業日」は 20=final・60=pending が並存するため提案まるごとではなく horizon ごとに final 判定する）。`score_pending_outcomes` 冒頭で既 final キー集合を 1 クエリ（`repo.list_finalized_outcome_keys`・`ix_proposal_outcomes_status` が効く）で引き、残り horizon が 0 本なら価格系列（`daily_quotes`/`index_quotes`）すら取らず即 return する。戻り値サマリ（`upserted`/`finalized`）は「今晩実際に採点した件数」の意味になる（final スキップ分は数えない＝毎晩の実処理量が見える）。**date 窓の二重ガード（古すぎる提案を除外）は今回見送り**＝残る単調増加は「永久 pending（上場廃止等で到達バーが永遠に来ない提案）」のみで、通常 pending は 60 営業日で final 化するレアケース。将来必要なら date 窓を後付けする。
  - **v1 は蒸留（reviewer 面→知識カード draft 化）をやらない**（次段＝テーマ B）。**提示ベース（手数料/スリッページ/実約定なし）であり実 P/L ではない**。`proposals.outcome`（承認結果）とも別列・別テーブルで分離。
- **理由**: 事後評価を Python が「事実」として焼き、AI は Tool 戻り値でのみ自分の成績を参照する（数字を push しない＝[ADR-014](#adr-014-ai-に数値を計算させないpython-が事実を計算し-ai-は判断する)/[ADR-025](#adr-025-画面コンテキストは軽量ヒントのみ数値はチャットに載せない)）。超過で hit を測るのは市場ベータと銘柄選択を分離するため（強気相場で全 buy が hit になる絶対リターンでは skill 評価にならない）。系列カウントは `train.py` 実績があり再現性が高い（[ADR-016](#adr-016-手法はテスト済みコードで実装する-ai-にその場で計算させない)）。自己改善は**自動チューニングではなく human-in-the-loop**＝policy/CORE には自動で触れず（[ADR-013](#adr-013-投資方針-policy-は単一を育てる)/[ADR-027](#adr-027-手法のパラメータは-ai-に触らせないコード定数として持つ)）、蒸留先は知識カードの draft に固定して人間承認を通す（[ADR-009](#adr-009-提案はするが発注はしない承認制)/[ADR-062](#adr-062-知識カード基盤に一本化する)）方針をテーマ B へ引き継ぐ。
- **代替案**: **(A) 実 P/L で採点**→未約定の提案には測れず、提示（提案）と実口座を混同する＝別テーブルで提示ベースに限定。**(B) 絶対リターンのみ**→相場全体の上げ下げと銘柄選択が混ざる＝超過を主に両方保存。**(C) `batch/calendar` で営業日カウント**→系列方式が休場を自然吸収し依存を増やさない＝系列採用。**(D) `proposals` に `source` 列追加**→スコープ拡大。生成元 journal から導出で足り、`journal_id` NULL は 'chat' に倒す（nightly 縮退の稀な誤ラベルは許容）＝v1 は列追加しない。**(E) CORE/プロンプトに数字を注入（push）**→[ADR-014](#adr-014-ai-に数値を計算させないpython-が事実を計算し-ai-は判断する)/[ADR-025](#adr-025-画面コンテキストは軽量ヒントのみ数値はチャットに載せない)に反する＝pull Tool に限定。
- **段階**: 実装（2026-07-02・ATDD＝受け入れテスト先行）。着手物＝① `quant/outcome.py`（純関数＋境界テスト）② `proposal_outcomes` ＋ migration `0036` ③ `db/repo/proposal_outcomes.py`（scorable 抽出・冪等 UPSERT・集計・recent）④ `services/track_record.py`（`score_pending_outcomes`/`get_track_record`）⑤ 夜間ジョブ `score_proposal_outcomes` ＋ `NIGHTLY_JOBS` 挿入 ⑥ Tool `get_track_record`（schemas/handlers/registry）⑦ CORE 一文 ⑧ テスト（quant 境界・service pending→final・job 冪等・tool 集計）⑨ docs 同期（本 ADR・[data-model.md](data-model.md)・[CLAUDE.md](../CLAUDE.md)）。backend pytest 1068 件 green・ruff／pyright green・migration を実 SQLite に適用し head=0036 を確認。**実 DB での初回採点（`^TPX`/`^SPX` 行の実在・提案が溜まってからの final 集計）は dev サーバでの実機確認に回す**（欠測時は `excess=NULL`＋fallback で動く）。スコープ外＝テーマ B（採点結果の知識カード蒸留＝reviewer 面）・画面/REST 露出（`GET /track-record`）・手法別 attribution（`proposals` に起点 signal を記録する拡張）。**追補（2026-07-02・採点入口の有界化）**＝当初実装の `list_scorable_*` は date 範囲も件数上限も final スキップも無い全件フルスキャンで、一度 final 化した提案も毎晩また採点し直していた。`proposal_outcomes` は冪等 UPSERT（`UNIQUE(origin_kind,origin_id,horizon)`）で行自体は増えないが、過去終値は不変なので final の再採点は完全な無駄で、提案が溜まるほど毎晩の価格取得＋採点計算が累積提案数に線形に膨らむ。対処＝`repo.list_finalized_outcome_keys` で既 final キー集合を 1 回引き、`_score_one` が残り horizon 0 本なら価格取得もせず即 return（決定の「採点入口の有界化」節）。**結果値は一切変わらない**（final の再計算は同じ値を上書きするだけ）安全な最適化。ATDD＝`test_track_record_service.py` に 4 本追加（final 再採点なし・horizon 別スキップ・全 horizon final は価格取得ごとスキップ・pending 回帰防止）。migration 不要。
- **関連**: [ADR-052](#adr-052-ニュース起点の売買アイデアを承認制で起票する)（buy/sell 起票＝採点母集団）・[ADR-067](#adr-067-夜-digest-の注目シグナルを合流ゲートai-選別に作り直す)（notable_picks・source 列）・[ADR-014](#adr-014-ai-に数値を計算させないpython-が事実を計算し-ai-は判断する)/[ADR-016](#adr-016-手法はテスト済みコードで実装する-ai-にその場で計算させない)（事実は Python・純関数）・[ADR-025](#adr-025-画面コンテキストは軽量ヒントのみ数値はチャットに載せない)（結果値を push しない）・[ADR-015](#adr-015-システムプロンプトは-core可変policy-の-2-層)（CORE は意図的コミット）・[ADR-013](#adr-013-投資方針-policy-は単一を育てる)/[ADR-027](#adr-027-手法のパラメータは-ai-に触らせないコード定数として持つ)（policy/手法パラメータは自動改変しない）・[ADR-009](#adr-009-提案はするが発注はしない承認制)/[ADR-062](#adr-062-知識カード基盤に一本化する)（テーマ B の蒸留先＝知識カード draft）・[ADR-002](#adr-002-sqlite-wal--upsert-で冪等に)（冪等 UPSERT）・[data-model.md](data-model.md)（`proposal_outcomes`）・`tasks/hermes-transfer-2026-07-02.md`（テーマ A の出所）。

## ADR-078: 判断ログを FTS5（trigram）で横断想起する読み取り Tool（帯域 n=1 を pull で広げる・D-1）

- **状況**: 夜AI・チャットとも「過去の自分の判断」を毎回 `get_recent_journal_summary(n=1)` の**直近 journal 1 件**しか参照できず（帯域 n=1・`advisor_journal` は embedding を持たず意味検索対象外）、却下理由や過去の帰結が次ターンに戻らない。テーマ A（[ADR-077](#adr-077-ai-の過去提案を市場結果で採点する夜間ループ提示ベースの銘柄選択スキル評価テーマ-a)）で「市場結果による事後採点（`proposal_outcomes`）」は入ったが、それは source×kind×horizon の**集計**（`get_track_record`）で、**個別の過去判断をキーワードで思い出す帯域**は無い。裏取りの結果、FTS5 は環境で利用可能（SQLite 3.51.3・`ENABLE_FTS5`・trigram tokenizer 可・拡張ロード不要）だが repo に FTS は未導入（既存の横断は embedding 意味検索のみ）。`tasks/hermes-transfer-2026-07-02.md` の ★2 D-1（hermes の `messages_fts` 想起の投資版）。
- **決定**:
  - **永続済みの判断ログ 3 ソース（`advisor_journal` / `proposals` / `notable_picks`）だけを FTS5（trigram＝CJK 部分一致・LLM/埋め込み不要）で索引**する。**生チャットは索引しない**（[ADR-029](#adr-029-会話は揮発-journal-昇格で残す) の会話揮発を守る＝永続した判断ログのみ対象）。索引する自由文列 = journal の `observations`＋`proposal`／proposals の `rationale`／notable_picks の `reason`（JSON 列 situation_briefing/body は生索引しない）。
  - **統合スタンドアロン 1 表 `judgment_fts(body, origin, ref_id, code, entry_date)`**（`origin`=journal/proposal/notable の判別列つき・external-content は使わない）。各基底表 → `judgment_fts` の**同期トリガ（AFTER INSERT/UPDATE/DELETE の 9 本）で自動同期**。検索は `MATCH`（フレーズ引用で FTS 演算子を無効化）＋`bm25()` 単一ランキング、`origin`/`code` は UNINDEXED 列への任意 exact 絞り。
  - **DDL の単一真実源を `app/db/fts.py`**（`ensure_judgment_fts`＝仮想表＋トリガを冪等作成／`rebuild_judgment_fts`＝全消し＋3 表から再 INSERT＝backfill/reindex 兼用／`drop_judgment_fts`）に置き、**alembic migration（本番）と `create_schema()`（テスト経路）の両方から呼ぶ**（FTS 仮想表/トリガは SQLAlchemy `metadata` に載らず `metadata.create_all` では作られないため・同じ DDL を二重に書かない）。migration `0037` の upgrade が `rebuild`（既存行を backfill）、downgrade が `drop`。
  - **read-only Tool `search_judgments`（`min_phase=1`・pull・tool_runs 非搭載＝[ADR-025](#adr-025-画面コンテキストは軽量ヒントのみ数値はチャットに載せない)）**を 1 本追加。ヒットは **bookend（目的→ヒット→帰結）** で返し、buy/sell 提案・注目選別は `proposal_outcomes` を `(origin_kind,origin_id)` で LEFT JOIN して horizon 20/60 の実現/超過リターン＋hit（採点済み）or pending を添える（journal はテキストのみ）。`get_track_record`（全体集計）と**個別想起**で補完。CORE 要素④に「類似局面を思い出したいとき `search_judgments`」の一文（意図的コミット＝[ADR-015](#adr-015-システムプロンプトは-core可変policy-の-2-層)）。
  - **常時注入の n=1 は据え置き**（`get_recent_journal_summary` は不変）。帯域拡張は pull Tool に限定し毎プロンプトのトークンを増やさない。
  - trigram の下限（3 文字未満は索引/検索不能）は service でガードして空＋理由に倒す。FTS 未作成/SQL 失敗も握って空＋理由（無人運用/チャットを落とさない＝[ADR-018](#adr-018-無人運用は既定で沈黙し失敗は握って続行する)・`search_news` 同型）。
- **理由**: 教師信号（過去判断とその帰結）を次ターンへ戻す帯域を、埋め込みコストゼロ・LLM 不要の全文検索で最安に広げる。生チャット非索引で [ADR-029](#adr-029-会話は揮発-journal-昇格で残す) の揮発意図を守り、索引対象は既に永続な列のコピーなので矛盾しない。統合スタンドアロン＋トリガは external-content 特有の delete 疑似行や 2 表のランキング融合を避け、コーパスが極小（1 日数行）ゆえテキスト重複の容量も無視できる。DDL 真実を 1 モジュールに集約し migration とテスト経路で共有することで「仮想表がテストで作られない」非対称を解消（[ADR-005](#adr-005-db-に触れるのは-fastapi-のみnext-は-rest-経由) の単一プロセス前提とも整合）。数字は Tool 戻り値でのみ渡す（[ADR-014](#adr-014-ai-に数値を計算させないpython-が事実を計算し-ai-は判断する)/[ADR-025](#adr-025-画面コンテキストは軽量ヒントのみ数値はチャットに載せない)）。
- **代替案**: **(A) `advisor_journal` に embedding 列を足して意味検索**→埋め込みコスト・夜間ジョブ追加・journal 1 日 1 件で母集団が薄く費用対効果が低い＝キーワード全文で足りる。**(B) テーブル別 external-content FTS（journal/proposals 別々）**→テキスト重複は無いが FTS 表 2〜3 本＋トリガの delete 疑似行構文＋bm25 スケールをまたぐ結果マージが要り壊れやすい＝統合スタンドアロン。**(C) FTS DDL を migration にインライン、テストは各自で作成**→DDL が二重化しドリフト＝単一関数を両経路で共有。**(D) 常時注入 n を広げる（push）**→全チャット/夜AI のトークンが恒常増＝pull＋CORE 一文で必要時のみ引く。**(E) notable_picks を索引対象外（journal＋proposals のみ・タスク字面どおり）**→ notable の `reason` は自由文＋採点済み帰結を持ち [ADR-077](#adr-077-ai-の過去提案を市場結果で採点する夜間ループ提示ベースの銘柄選択スキル評価テーマ-a) と母集団が非対称になる＝3 ソース採用。**(F) vec0 仮想表へ昇格**→規模が育つまで不要（[ADR-045](#adr-045-ニュース意味検索を-embedding--vecdistancecosine-で行うvec0-は後回し) 同方針・段階 C は将来）。
- **段階**: 実装（2026-07-02・ATDD）。着手物＝① `app/db/fts.py`（DDL 単一真実源＝ensure/rebuild/drop）② `create_schema()` 拡張（FTS を立てる）③ `db/repo/judgments.py`（`search_judgment_fts`＝MATCH+bm25／`search_judgments`＝origin 別 bookend＋`proposal_outcomes` 合流）④ `services/judgments.py`（3 文字ガード・JSON-safe 整形・例外→空＋理由）⑤ Tool `search_judgments`（schemas/registry/handlers・`min_phase=1`）⑥ migration `0037_judgment_fts`（rebuild/drop）⑦ CORE 一文＋ドリフトガード更新 ⑧ テスト（repo=trigram/トリガ同期/フィルタ/rebuild/bm25・service=bookend/JSON-safe/ガード/空・tool=handler）⑨ docs 同期（本 ADR・[data-model.md](data-model.md)・[CLAUDE.md](../CLAUDE.md)・`tasks/hermes-transfer-2026-07-02.md` §7）。backend pytest 1082 件 green・ruff／pyright green・migration を実 SQLite に通し（0036→行投入→0037 で backfill 3 行→trigram MATCH→トリガ同期→downgrade で完全消去）確認。**dev サーバで AI が実際に `search_judgments` を呼び bookend が返るかの E2E は次回運用時に確認**。スコープ外＝D-1 に非依存の他束（テーマ B 蒸留 reviewer 面・投資家プロファイル記憶層〔[ADR-082](#adr-082-投資家プロファイル記憶層は独立テーブル承認制の傾向メモで持つ記述と規範を分離)〕・学習グラフ・cron）・段階 C（FTS×embedding ハイブリッド）・常時注入 n 拡張。
- **関連**: [ADR-077](#adr-077-ai-の過去提案を市場結果で採点する夜間ループ提示ベースの銘柄選択スキル評価テーマ-a)（採点済み帰結＝bookend の合流元・同じ母集団を個別想起）・[ADR-029](#adr-029-会話は揮発-journal-昇格で残す)（生チャット非索引の線引き）・[ADR-045](#adr-045-ニュース意味検索を-embedding--vecdistancecosine-で行うvec0-は後回し)（横断検索の既存作法・vec0 後回しの同方針）・[ADR-025](#adr-025-画面コンテキストは軽量ヒントのみ数値はチャットに載せない)/[ADR-014](#adr-014-ai-に数値を計算させないpython-が事実を計算し-ai-は判断する)（pull・数字は Tool 戻り値）・[ADR-018](#adr-018-無人運用は既定で沈黙し失敗は握って続行する)（失敗は空＋理由）・[ADR-005](#adr-005-db-に触れるのは-fastapi-のみnext-は-rest-経由)（DB は FastAPI 1 プロセス）・[ADR-002](#adr-002-sqlite-wal--upsert-で冪等に)（冪等）・[data-model.md](data-model.md)（`judgment_fts`）・`tasks/hermes-transfer-2026-07-02.md`（D-1 の出所）。

## ADR-079: 清原式ネットキャッシュを screen 指標として実装し、手法カードを kind で「戦略カード」に一般化する

- **状況**: 清原達郎『わが投資術』の「低 PER×小型株×ネットキャッシュ比率≥1」を AI に使わせたいという要望。裏取りの結果、(a) ネットキャッシュ比率＝(流動資産＋投資有価証券×0.7−総負債)÷時価総額は、時系列イベント（`signals`）ではなく**財務断面のスクリーニング**であり、既存 `screen_valuation`/`screen_us_valuation`（[ADR-031](#adr-031-株式スクリーナー夜間-valuation_snapshots--読み取り時ランク市場ごとに分離)/[ADR-048](#adr-048-銘柄バリュエーション判断基準roeperpbrを-tool事実参照知識カードで持たせる)）の自然な拡張であること、(b) 分子の BS 3 項目は US=yfinance balance_sheet（`Current Assets`/`Investments And Advances`/`Total Liabilities Net Minority Interest`）でフル式が組め、JP=edinetdb.jp（[ADR-064](#adr-064-売掛在庫の質業績の質-2-は-edinetdbjp-の構造化財務で実装し接続設定を-dbwebui-化する)）は流動資産・総負債は取れるが投資有価証券の専用フィールドが無いこと、が分かった（※この「専用フィールド無し」は**後日の追補で誤りと判明**＝edinetdb.jp の OpenAPI に `investment_securities` が実在。下記 追補参照）。加えて、この「確立した外部手法の知識」をどこに置くかで、当初は knowledge_cards（[ADR-062](#adr-062-知識カード基盤に一本化する)）を案としたが、ユーザーから「アプリ組み込みの手法で、UI 編集は要らない（正典として固定したい・stale なら消す）」との指摘があり、手法カード（[ADR-075](#adr-075-手法カードをリポジトリ所有の第-4-知識源にするsignal_type-キーオンデマンド注入linked_signal_type-非推奨化)）の器が適切と判明した。ただし手法カードは `signal_type` に 1:1 で縛られ、signal を持たない screen 手法は orphan として弾かれる。
- **決定**:
  - **計算は `quant/valuation.py` の純関数 `net_cash(current_assets, investment_securities, total_liabilities)`**（流動資産＋投資有価証券×0.7−総負債・[ADR-014](#adr-014-ai-は計算しないtool-calling-原則rag-は後付け)/[ADR-016](#adr-016-手法はコードで実装する手法db-は索引でありコードの代替ではない)）。**JP は投資有価証券を渡さない＝簡略式（流動資産−総負債）で v1**（投資有価証券を切り捨てるぶん保守的に過小評価へ倒れる）。US はフル式。JP の full 化（公式 EDINET の `jppfs_cor:InvestmentSecurities`）は別 PR＝`tasks/net-cash-fullization-todo.md`（→**下記 追補で edinetdb.jp の `investment_securities` により達成・公式 EDINET は不要と判明**）。
  - **`net_cash`（絶対額）は物理列に焼く**（両 `valuation_snapshots`/`us_valuation_snapshots`・migration `0038`）。**`net_cash_ratio`（÷時価総額）は物理列にせず read-time で導出**（repo の inner subquery で `net_cash / market_cap`＝`per_sector_pctile`/`market_cap_rank` と同じ read-time 方式・[ADR-031](#adr-031-株式スクリーナー夜間-valuation_snapshots--読み取り時ランク市場ごとに分離)）。理由: `net_cash` は BS 由来で四半期ごとにしか動かないが時価総額は日次で動くため、週次 cadence のジョブで比率を焼くと最大 7 日古い時価総額の比率になる（鮮度バグ・[ADR-071](#adr-071-is_delayed-をプランの仮定からas_of-の鮮度実測に一元化するai-は-tool-の遅延フラグを見る) の精神）。SQLite は分母 0/NULL で除算結果を NULL にする＝quant のガード（None）と一致。
  - **焼き込みは #2 売掛/在庫の質（[ADR-064](#adr-064-売掛在庫の質業績の質-2-は-edinetdbjp-の構造化財務で実装し接続設定を-dbwebui-化する)）の夜間ジョブに相乗り**（`calc_receivables_inventory`/`calc_us_receivables_inventory`・同じ edinetdb.jp/yfinance 経路・cadence/月予算ガード共有・新ジョブなし）。共通 service `edinetdb_quality.compute_quality_from_financials` の返り dict に `net_cash` を足し、当期選定 guard を「受取債権/在庫/流動資産/総負債のいずれかがあれば処理」に緩める（BS だけの銘柄も net_cash を拾う・#2 列は None のまま）。repo の `_RECV_INV_COLUMNS`/`_US_RECV_INV_COLUMNS` に `net_cash` を追加し、日次 `calc_valuation` の partial UPSERT（[ADR-002](#adr-002-データベースは-sqlitewal-モード)・#1/#7）で温存する。
  - **Tool は既存を拡張**（新 Tool なし）。`screen_valuation`/`screen_us_valuation` に `net_cash_ratio_min`・`net_cash_min`・`market_cap_max`（小型株上限）を追加、`get_valuation`/`get_us_valuation` の返却に `net_cash`/`net_cash_ratio` を中継（verdict は持たない・解釈は LLM＝[ADR-014](#adr-014-ai-は計算しないtool-calling-原則rag-は後付け)）。
  - **手法カード（method_cards）を kind で一般化**（[ADR-075](#adr-075-手法カードをリポジトリ所有の第-4-知識源にするsignal_type-キーオンデマンド注入linked_signal_type-非推奨化) の拡張）。frontmatter に `kind`（既定 `signal`）を導入し、**`kind: strategy`＝signal を持たない手法**（ファイル名＝手法スラッグ・ドリフト検査の orphan/missing 対象外）を許す。清原式は `method_cards/net_cash_value.md`（`kind: strategy`）＝リポジトリ所有・アプリ非編集・progressive disclosure（`get_method_card('net_cash_value')`＋カタログに `[strategy]` タグ）。**「編集不要の正典＝method_cards／育てる知識＝knowledge_cards」**の住み分け（＝誰が所有し編集するか）を明文化し、当初案の「外部手法は knowledge_cards」を上書きした。
- **理由**: 清原式は財務断面のスクリーニングなので signal 化（毎晩発火）は不自然で、screen 拡張が設計に素直。事実（net_cash）は Python が焼き、比率は鮮度優先で read-time 導出、良し悪しは LLM が解釈（[ADR-014](#adr-014-ai-は計算しないtool-calling-原則rag-は後付け)）。#2 と同型経路に相乗りすることで新テーブル・新ジョブ・接続設定を増やさない。手法カードの kind 一般化は、器を増やさず（method の 3 分裂を解消した [ADR-075](#adr-075-手法カードをリポジトリ所有の第-4-知識源にするsignal_type-キーオンデマンド注入linked_signal_type-非推奨化) の精神を維持）「リポジトリ所有・不変の手法知識」という本質に沿ってスコープを広げるもの。計算は依然 `quant`（[ADR-016](#adr-016-手法はコードで実装する手法db-は索引でありコードの代替ではない)）でカードは解釈のみ。
- **代替案**: **(A) net_cash を signal 化して既存 method_cards に載せる**→断面手法を毎晩発火させるのは不自然（同じ割安銘柄が延々出続ける）＝screen。**(B) net_cash_ratio も物理列に焼く（両列）**→週次 cadence で日次の時価総額が最大 7 日古くなる＝read-time 導出。**(C) 外部手法を knowledge_cards に置く**→UI 編集・triage・育てる器はミスマッチ（編集不要の正典）＝method_cards（strategy）。**(D) strategy 専用の新ディレクトリ/器を新設**→ほぼ同一機構の二重化＝method_cards を kind で一般化。**(E) JP も公式 EDINET でフル式 v1**→ XBRL 数値パースが重く鍵未整備＝v1 は簡略式・full 化は別 PR（→**追補で公式 EDINET を使わず edinetdb.jp の `investment_securities` により達成**＝この代替案の前提〔専用フィールド無し〕自体が誤りだった）。
- **段階**: 実装（2026-07-02・ATDD＝受け入れテスト先行）。着手物＝① `quant.net_cash`（純関数＋境界テスト）② migration `0038_net_cash`（両 snapshot に `net_cash`・head=0037→0038）③ adapters（edinetdb `_normalize_financial`＋us_equity `fetch_balance_sheet` に BS 項目）④ service（`compute_quality_from_financials` に net_cash・guard 緩和）⑤ repo（subquery に read-time `net_cash_ratio`＋screen range/sort＋`_RECV_INV_COLUMNS`）⑥ Tool（screen 引数＋get_valuation 中継＋description 橋渡し）⑦ method_cards の kind 一般化＋`net_cash_value.md`＋ドリフト検査除外 ⑧ テスト（quant/service/tool 読み取り時導出フィルタ/partial 温存/JP・US ジョブ/method_cards）⑨ docs 同期（本 ADR・[data-model.md](data-model.md)・[advisor.md](advisor.md)・[CLAUDE.md](../CLAUDE.md)・`method-card-authoring` スキル）。backend pytest 1099 件 green・ruff／pyright green・migration を実 SQLite に適用し head=0038（両 snapshot に `net_cash`・`net_cash_ratio` は物理列なし）を確認。**dev サーバでの実データ焼き込み（edinetdb.jp の実キー名確認・US net_cash_ratio の妥当値）と AI の screen 実行 E2E は次回運用時に確認**（edinetdb.jp の BS キー名は公開仕様ベースで、取れなければ None に倒れ net_cash も None＝安全）。スコープ外＝JP full 化（`tasks/net-cash-fullization-todo.md`・→**下記 追補で達成**）・銀行/保険など BS 特殊業種の扱い・画面（frontend スクリーナー UI への net_cash_ratio 露出）。
- **関連**: [ADR-064](#adr-064-売掛在庫の質業績の質-2-は-edinetdbjp-の構造化財務で実装し接続設定を-dbwebui-化する)（相乗り経路・同型の夜間フロー）・[ADR-075](#adr-075-手法カードをリポジトリ所有の第-4-知識源にするsignal_type-キーオンデマンド注入linked_signal_type-非推奨化)（一般化した親 ADR・kind 導入）・[ADR-062](#adr-062-知識カード基盤に一本化する)（knowledge_cards との住み分けを再確認）・[ADR-031](#adr-031-株式スクリーナー夜間-valuation_snapshots--読み取り時ランク市場ごとに分離)（screen＋read-time ランクの既存作法）・[ADR-048](#adr-048-銘柄バリュエーション判断基準roeperpbrを-tool事実参照知識カードで持たせる)（valuation Tool・事実のみ）・[ADR-071](#adr-071-is_delayed-をプランの仮定からas_of-の鮮度実測に一元化するai-は-tool-の遅延フラグを見る)（鮮度優先で read-time 導出）・[ADR-014](#adr-014-ai-は計算しないtool-calling-原則rag-は後付け)/[ADR-016](#adr-016-手法はコードで実装する手法db-は索引でありコードの代替ではない)（事実は Python・カードは解釈）・[ADR-002](#adr-002-データベースは-sqlitewal-モード)（partial UPSERT で温存）・[data-model.md](data-model.md)（`net_cash`）・`tasks/net-cash-fullization-todo.md`（JP full 化）。
- **追補（2026-07-02・JP full 化）**: 上の「JP は簡略式で v1・full 化は公式 EDINET の XBRL＝別 PR」という前提を**上書きする**。裏取りの結果、当初「edinetdb.jp には投資有価証券の専用フィールドが無い」としたのは**誤り**で、edinetdb.jp の OpenAPI（`https://edinetdb.jp/v1/openapi.yaml`・147 フィールド）に **`investment_securities`（投資有価証券・円）が実在**した（現アダプタ `_normalize_financial` が読んでいなかっただけ）。よって **公式 EDINET の XBRL パーサも 401 疑いの公式キーも一切不要**で、`calc_receivables_inventory` が既に叩く**同じ `/financials` レスポンス**から 1 キー抽出するだけで JP もフル式になった（追加フェッチ ゼロ）。**写像は `investment_securities`（非流動）のみ×0.7**＝`short_term_securities`（有価証券・流動）は既に `current_assets` に含まれ二重計上になるため足さず、`investments_and_other_assets`（親項目・長期貸付金/敷金/繰延税金資産を含む）は過大評価になるため使わない（US の `Investments And Advances`＝非流動と対称）。欠落（IFRS 銘柄・古い年）時のみ簡略式（保守側）に自然フォールバック。実体＝`adapters/edinetdb._normalize_financial` に `investment_securities` を追加（1 行）＋各所の docstring/コメント更新のみ（quant/service/物理列/比率導出/screen/Tool は既にフル式対応済みで無改変）。migration 不要（`net_cash` 列は 0038 で既存）。ATDD＝`tests/test_edinetdb.py`（正規化で投資有価証券を写像・欠落は None）＋`tests/test_edinetdb_quality.py`（フル式/欠落フォールバック）で先行検証。実データ焼き込み差分（代表銘柄で簡略式→フル式）と単位裏取りは dev 実機プローブで確認。

## ADR-080: チャットからのウォッチリスト追加は「AI が候補提示＋UI が追加」に分離する（propose_watchlist は検証 only・surfacing は昼だけ）

- **状況**: AI Advisor が「本命 37120 情報企画／次点 61380…」のように厳選ショートリストを提示しても、ユーザーは手で `/watchlist` を開いて 1 件ずつ `POST /watchlist`（[ADR-020](#adr-020-銘柄ドシエは-db-に保存しリポジトリ-markdown-に置かない)）するしかない。会話の流れで候補を選んでそのままウォッチ入りさせたい、という要望。ユーザーの明示方針＝**(a) ウォッチ追加は UI 側でやりたい（AI に勝手に watchlist を書かせたくない）**・**(b) 仮にこれが Tool になるとして、夜バッチで AI がその Tool を使い「ユーザーの選択待ちで止まる」ようなことは起きてほしくない**。裏取りの結果、(b) の「止まる」はこのアーキでは原理的に起きない（Tool は同期呼び出しで即返る・人間入力を待つ一時停止機構は無い）ことが分かった。恐れるべきは「止まる」ではなく「Tool が副作用（勝手に watchlist 追加）を持つこと」で、そこを設計で殺せば懸念は消える。既存の対称形＝`submit_notable_stocks`/`notable_picks`（[ADR-067](#adr-067-夜-digest-の注目シグナルを合流ゲートai-選別で作り直す)）が「AI が候補を挙げ／永続は呼び出し側ごとに配線（夜だけ）」という型を確立している。
- **決定**:
  - **検証 only の Tool `propose_watchlist(candidates=[{code, reason}])`**（`min_phase=4`）を足す。**watchlist への追加はしない**＝候補（code＋reason）を構造化して渡すだけ。社名は AI に運ばせず backend が `stocks` で解決し、未知/US コードは drop（[ADR-018](#adr-018-夜間バッチの障害設計失敗は握って通知後続を止めない)・watchlist は JP 専用）。reason は任意で、欠落しても候補は落とさず `reason=""` で残す（銘柄を取りこぼさない）。
  - **候補の surfacing（UI へ載せる処理）は昼 `router` だけに配線する**。読み取り専用ブリッジ `journaling.build_watchlist_candidates_from_tool_runs`（`persist_*_from_tool_runs` と同型だが**何も永続しない**）が tool_runs の args から `[{code, company_name, reason}]` を組み、`/chat` の `ChatResponse.watchlist_candidates` に載せる（`card_ids`/`journal_id` と同型のインライン・フィードバック＝[ADR-065](#adr-065-ai-advisor-の大画面化と知識ノート壁打ち導線)）。**夜 nightly はこの surfacing を配線しない**ので、夜 AI が propose_watchlist を呼んでも返り値を捨てる **no-op**（懸念 (b) を構造で保証＝`submit_notable_stocks` の鏡像：あちらは夜だけ永続・こちらは昼だけ surfacing）。
  - **追加はユーザーが UI で行う**（[ADR-009](#adr-009-ai-の提案は承認制人間が承認するまで実行しない) の承認制精神）。frontend は候補を **assistant バブル（`Msg`）に括り付けて localStorage 永続**し（フローティングと `/advisor` は同じ `ChatConversation` を使うので 1 個で両画面に出る・遡っても選べる）、チェックリストで見せる。描画時に `GET /watchlist` でライブ照合し既存銘柄は ✓ 追加済み（押せない）にする。選んで「追加」で各 code を既存の `postWatchlist(code, reason)`（冪等 do_nothing）で入れ、reason を watchlist の `note` に焼く。
  - **CORE 要素④に発火条件を散文追記**＝厳選ショートリスト提示時だけ propose_watchlist・追加は UI・AI は書かない・雑談の言及では呼ばない（[ADR-062](#adr-062-知識カード基盤に一本化する)「規律は CORE へ吸収」・propose_card の「乱発しない」精神）。
- **理由**: (a)(b) の両方を同時に満たす最小構成。Tool を副作用ゼロ（args を router が拾うだけ・永続なし）にすれば「夜で止まる／勝手に追加」は原理的に不可能。追加を既存 `POST /watchlist`（冪等）に一本化することが「UI で追加」という要望そのもの。実装は `submit_notable_stocks`/`propose_trade`＋`persist_*_from_tool_runs`＋`card_ids` surfacing の確立パターンに完全に乗り、新テーブル・新 migration が要らない。
- **代替案**: **(A) フロントが reply 本文を正規表現で銘柄コード抽出**→ AI の文面フォーマットに依存して脆い（コード表記ゆれ・社名取り違え・US 混入で崩れる）＝構造化 Tool。**(B) AI が `add_to_watchlist` で即追加**→ ユーザーの「UI で追加」に反し [ADR-009](#adr-009-ai-の提案は承認制人間が承認するまで実行しない) 承認制の精神に反する・夜が勝手に追加する余地も生む＝提示のみ。**(C) 候補を pending 台帳に永続し `/proposals` 的な別画面で後日承認**→ ユーザーは「チャット内で選ぶ」体験を求めており重い＝会話バブル括り付けの ephemeral（localStorage 永続で十分）。**(D) 夜 nightly にも propose_watchlist を露出しない（face ゲート新設）**→ face 別 Tool ゲート機構が今は無く新設はスコープ増・no-op で実害は無い＝露出はそのまま surfacing を昼だけに。
- **段階**: 実装（2026-07-02・ATDD＝受け入れテスト先行・`/grill-me` で設計合意）。着手物＝① schemas（`WatchlistCandidateArg`/`ProposeWatchlistArgs`）② handlers（`handle_propose_watchlist`・read-only 検証）③ registry（`ToolDef` min_phase=4）④ journaling（`build_watchlist_candidates_from_tool_runs`・永続しない）⑤ router（`ChatResponse.watchlist_candidates`＋has_watchlist のときだけ read 接続で載せる）⑥ CORE 要素④ 追記 ⑦ frontend（`advisor.ts` 型・`advisor-chat-context` の `Msg` 括り付け・`WatchlistCandidatePicker.tsx` 新規・`ChatConversation` でバブル下に描画）⑧ テスト（`tests/test_watchlist_candidates.py`＝build/handler/router surfacing）⑨ docs 同期（本 ADR・[api.md](api.md)・[advisor.md](advisor.md)・[CLAUDE.md](../CLAUDE.md)）。backend pytest green・ruff／pyright green・frontend biome／tsc green。migration 不要（既存 `watchlist` 表・`POST/GET /watchlist` を流用）。**手動 E2E（実チャットでフローティングと `/advisor` の両方に候補が出る→選択→追加→`/watchlist` 反映→リロードで ✓ 永続）は次回運用時に確認**。スコープ外＝追加後の `/watchlist` 画面リアルタイム反映（次回訪問で反映・チャット内は楽観✓）・追加前の note 編集 UI（AI reason をそのまま焼く）・US 銘柄のウォッチ（watchlist は JP 専用のまま）。
- **関連**: [ADR-067](#adr-067-夜-digest-の注目シグナルを合流ゲートai-選別で作り直す)（`submit_notable_stocks`/`notable_picks`＝「候補提示＋呼び出し側ごとに配線」の鏡像）・[ADR-052](#adr-052-ニュース起点の-buysell-アイデアを-propose_trade-で承認制起票する)（`propose_trade`＝検証 only Tool＋`persist_*_from_tool_runs`）・[ADR-065](#adr-065-ai-advisor-の大画面化と知識ノート壁打ち導線)（`ChatResponse.card_ids` の surfacing・`/advisor` とフローティングの会話共有）・[ADR-009](#adr-009-ai-の提案は承認制人間が承認するまで実行しない)（追加は人間＝承認制）・[ADR-018](#adr-018-夜間バッチの障害設計失敗は握って通知後続を止めない)（未知 code drop・per-item グレースフル）・[ADR-020](#adr-020-銘柄ドシエは-db-に保存しリポジトリ-markdown-に置かない)（`watchlist`・`POST /watchlist`）・[ADR-014](#adr-014-ai-は計算しないtool-calling-原則rag-は後付け)（AI は社名解決させない・事実は Python）・[api.md](api.md)（`ChatResponse.watchlist_candidates`）・[advisor.md](advisor.md)（`propose_watchlist`）。

## ADR-081: 経験蒸留は reviewer 面が採点済み outcome を知識カード draft に蒸留する（Tool-calling・活動量ゲート・migration 不要）

- **状況**: 自己改善ループ（hermes-transfer・`tasks/hermes-transfer-2026-07-02.md`）の骨格は ① 判断を残す（既存）② 市場が採点（[ADR-077](#adr-077-ai-の過去提案を市場結果で採点する自己改善ループproposal_outcomes--get_track_record)）③ 成績を戻す（`get_track_record`/`search_judgments`＝[ADR-078](#adr-078-判断ログ横断想起は統合-fts5-仮想表-judgment_fts--tool-search_judgments-で行う)）まで通ったが、**④ 知識へ蒸留**（採点結果を次に効く知識に変える段）が未配線。今は採点済み outcome（proposal_outcomes）が台帳に貯まるだけで、AI が「自分は volume_spike 起点の buy を 60 営業日で当てられていない」等の傾向を durable な知識として持てない（毎回 pull で引き直すしかない）。hermes の「会話を読み直して skill を書き換える」自己改善を、AssetVane では **市場結果で採点した傾向を知識カード draft に落とし人間が承認する**形へ翻訳する（教師信号は会話でなく市場＝[ADR-014](#adr-014-ai-は計算しないtool-calling-原則rag-は後付け) と噛み合う）。設計は `/grill-me` で 13 分岐を詰めた（`tasks/hermes-transfer-2026-07-02.md` §8）。裏取りの結果、reviewer に要る Tool は全て既存（`get_track_record`/`search_judgments`/`search_cards`/`propose_card`/`adjust_card_weight`）で新 Tool 不要、reviewer 面は FACES tuple に足すだけ（seed migration 無し）、カーソルは既存 `fetch_meta` 流用、`source='reviewer'` は文字列値＝**新テーブル/新列ゼロ**と分かった。
- **決定**:
  - **蒸留は Tool-calling ターン（`run_turn`・新 `reviewer` 面）で回す**（§8 Q1）。単発 JSON でなくエージェント形にする核＝**起票前に `search_cards` で近傍カードを検索し重複 draft を作らない dedup 規律**を持たせるため（[ADR-062](#adr-062-知識カード基盤に一本化する) の意味検索を蒸留にも効かせる）。既存 `run_turn`＋`persist_card_ops_from_tool_runs`（[ADR-065](#adr-065-ai-advisor-の大画面化と知識ノート壁打ち導線)）を再利用し、暴走/コストは活動量ゲート＋toolset 制限＋`max_rounds` で抑える。
  - **reviewer は最小 toolset だけ見せ、末尾で `persist_card_ops` のみ実行する（多重防御）**（§8 Q2）。`REVIEWER_TOOLSET = {get_track_record, search_judgments, search_cards, propose_card, adjust_card_weight}`。`openai_tools(phase, *, allow=…)`／`run_tool_loop`／`run_turn` に任意 `tool_names` allowlist を通し（既定 `None`＝全 Tool で chat/nightly 不変）、reviewer だけ集合を渡す。`propose_trade`/`submit_journal`/`submit_notable_stocks` は見せない＝無駄ラウンドと誤起票を断ち、persist 層（card_ops だけ拾う）が backstop。
  - **活動量ゲート＝カーソル＋閾値**（§8 Q3・[ADR-018](#adr-018-夜間バッチの障害設計失敗は握って通知後続を止めない) の「新しく解決した経験が溜まった晩だけ学ぶ」）。`fetch_meta('reviewer:cursor')` に「最後にレビューした採点時刻」を持ち、**新規 final outcome の件数が閾値（既定 3・config `reviewer_min_new_finals`）未満なら LLM を呼ばず skip**。成功時のみカーソルを前進（skip/失敗では据え置き＝材料を失わない）。**「新規 final」は `as_of_date` でなく `scored_at`（採点時刻）で数える**＝[ADR-071](#adr-071-is_delayed-をプランの仮定からas_of-の鮮度実測に一元化するai-は-tool-の遅延フラグを見る) の精神。理由: Free/Light の鮮度遅延で horizon 到達日（`as_of_date`）が過去日でも finalize は後日になり得るため、`as_of_date > cursor` だと「遅れて final 化した行」を取りこぼす。`scored_at` は final 化時刻で、採点入口の有界化（[ADR-077](#adr-077-ai-の過去提案を市場結果で採点する自己改善ループproposal_outcomes--get_track_record) で final は再採点しない）ゆえ一度立つと不変＝「前回レビュー以降に新しく我々が知った outcome」の正しい信号。
  - **過学習の足切りは Python が頻度カウントで担保**（§8 Q4）。教材の集計パートは `aggregate_track_record` の **count ≥ `min_samples`（既定 3・config `reviewer_min_samples`）の source×kind×horizon バケットだけ**を「傾向」として提示し、個別 outcome は文脈としてのみ渡す。プロンプトに「単発トレードから durable card を作るな・一般化はバケットを引用せよ」と明記（最小サンプルの担保は LLM に委ねず整形段で断つ）。
  - **教材の中身は Python 計算の verbatim ＋ bookend**（§8 Q5・[ADR-025](#adr-025-画面コンテキストは軽量ヒントのみチャットに数値を載せない)）。① count≥floor の `get_track_record` 集計 ② カーソル以降の新規 final を各 outcome ごとに「起点 proposal.rationale／notable.reason（頭）→ 採点数値（尾）」で bookend ③ 直近 journal 少量。数値は全て Python が計算した値をそのまま。生チャットは載せない（[ADR-029](#adr-029-生チャットは-fts5-索引しない揮発)）。足りなければ reviewer が `get_track_record`/`search_judgments`/`search_cards` で pull で深掘る。
  - **draft 固定＋source を決定論で強制**（§8 Q6・[ADR-009](#adr-009-ai-の提案は承認制人間が承認するまで実行しない)）。`persist_card_ops_from_tool_runs` に任意 `source_override` を足し、reviewer job は `'reviewer'` を渡す（LLM の source 引数を信用しない・既存 nightly/chat は `None` で不変）。カードは常に `status='draft'`、活性化は人間が `/cards` で（reviewer 由来 draft を source で識別・選別できる）。
  - **重複回避/既存改訂の v1 スコープ**（§8 Q7）。v1＝`search_cards` 先行→近傍かつ有効なら `adjust_card_weight`↑／近傍だが誤りなら↓／新規なら `propose_card`。**既存カードの本文改訂は v1 スコープ外**（本文改訂 Tool は作らない・near-dup 判定は意味検索＝LLM 判断で Python 自動マージなし＝themes の前例）。
  - **ジョブ `distill_experience` を `score_proposal_outcomes` の後・`notify_cost_warn`/`notify_digest` の前に挿入**（§8 Q8）。その晩の final が採点された直後に蒸留する（NIGHTLY_JOBS の唯一の backward-looking ペア）。
  - **digest フィードバック＋障害設計**（§8 Q9/Q11）。当夜 reviewer が draft を作ったら `notify_digest` に決定論の 1 行「🗂 知識ノート下書き N 件（/cards で確認）」を足す（`card_ids` フィードバックと同型）。face 未設定＝**沈黙 skip（ok=True・triage/tagger 同型）**、LLM ハード失敗＝job が ok=False で runner 集約通知（非致命・後続と digest は止めない）。skip/失敗時はカーソルを前進させない。
  - **「レビュー済み」状態は新表/新列を作らず `fetch_meta('reviewer:cursor')` の 1 本**（§8 Q13・`edinet:crawl` 前例）。**migration 不要**。
- **理由**: ループの ④ を全 ADR を破らず閉じる最小構成。教師信号は市場採点のまま（[ADR-014](#adr-014-ai-は計算しないtool-calling-原則rag-は後付け)）、蒸留先は人間承認の draft に固定（[ADR-009](#adr-009-ai-の提案は承認制人間が承認するまで実行しない)/[ADR-062](#adr-062-知識カード基盤に一本化する)）、policy/CORE は自動改変しない（[ADR-013](#adr-013-投資方針-policy-は単一を育てる)/[ADR-015](#adr-015-システムプロンプトは不変-core--可変-policy-に分離)＝自己チューニングの一線）、生チャットは索引しない（[ADR-029](#adr-029-生チャットは-fts5-索引しない揮発)）。既存 `run_turn`/`persist_card_ops`/`fetch_meta`/`aggregate_track_record` を再利用し新スキーマ・新 Tool ゼロで、★1・★2 より低リスク。過学習は Python の頻度カウント（floor）と活動量ゲートで二重に抑える。
- **代替案**: **(A) 単発 JSON（`generate_once`）で蒸留**→ `search_cards` で既存カードを見に行けず重複 draft を量産・dedup 規律を失う＝Tool-calling ターン。**(B) 毎晩無条件に蒸留**→ 材料 1 件でも呼び過学習・承認疲労・コスト増＝活動量ゲート（新規 final ≥ 閾値）。**(C) カーソルを `as_of_date` で数える**→ 鮮度遅延で遅れて final 化した行を取りこぼす＝`scored_at`（final 化時刻）で数える。**(D) reviewer がカードを直接 active 化**→ [ADR-009](#adr-009-ai-の提案は承認制人間が承認するまで実行しない) の承認制に反する＝常に draft・人間が `/cards`。**(E) 「レビュー済み」用の新列/新表**→ スキーマ増＝`fetch_meta` カーソル 1 本で足りる。**(F) reviewer に policy/CORE を書き換えさせる**→ 自己チューニングの一線を越える（[ADR-013](#adr-013-投資方針-policy-は単一を育てる)/[ADR-015](#adr-015-システムプロンプトは不変-core--可変-policy-に分離)）＝蒸留先は knowledge_cards draft に固定。
- **段階**: 実装（2026-07-03・ATDD＝受け入れテスト先行・`/grill-me` で設計合意）。着手物＝① `llm_config.FACES` に `reviewer` 追加（seed migration なし・conftest 自動 seed・`/settings` 自動露出）② `openai_tools`/`run_tool_loop`/`run_turn` の `tool_names` allowlist＋`REVIEWER_TOOLSET` 定数 ③ repo `list_new_final_outcomes(since)`（final・`scored_at > since`・rationale/reason bookend）④ service `experience.py`（ゲート件数・素材構築＝count≥floor 集計＋新規 final bookend＋直近 journal・format）⑤ `advisor/reviewer.py`（`run_experience_distillation`＝ゲート→`build_messages`→`run_turn(source='reviewer', tool_names=REVIEWER_TOOLSET)`→`persist_card_ops(source_override='reviewer')`→カーソル前進）⑥ `persist_card_ops_from_tool_runs` に `source_override` ⑦ job `distill_experience`（begin 境界所有・FaceNotConfigured→ok=True 沈黙 skip・他例外→ok=False）＋NIGHTLY_JOBS 挿入 ⑧ `notify_digest` に reviewer draft N 件の 1 行 ⑨ テスト（ゲート・素材 floor・カーソル前進/据え置き・source 強制・allowlist 5 本・job 順序/冪等・digest 行・FACES 6 化）⑩ docs 同期（本 ADR・[advisor.md](advisor.md)・[data-model.md](data-model.md)・[CLAUDE.md](../CLAUDE.md)・`tasks/hermes-transfer-2026-07-02.md` §7/§8。自己改善ループは roadmap の Phase でなく `tasks/hermes-transfer` が正本ゆえ roadmap.md は触らない）。backend pytest green・ruff／pyright green・**migration 不要（head 不変）**。**実データ運用（reviewer 面を `/settings` で登録＋pipeline 再稼働で採点済み outcome が閾値ぶん貯まる＋AI が実際に Tool を呼ぶ E2E）は ★1・★2 と同じボトルネック（dev は LLM 未設定・提案 0 件）で次回運用時に確認**。スコープ外＝既存カード本文の改訂 Tool（v1 は weight 調整のみ）・学習グラフ可視化（★5・別 ADR）・投資家プロファイル記憶層（★4・[ADR-082](#adr-082-投資家プロファイル記憶層は独立テーブル承認制の傾向メモで持つ記述と規範を分離) で実装＝独立テーブル・level=user は不採用）。
- **関連**: [ADR-077](#adr-077-ai-の過去提案を市場結果で採点する自己改善ループproposal_outcomes--get_track_record)（採点＝教師信号・`scored_at`/`aggregate_track_record` を教材に）・[ADR-078](#adr-078-判断ログ横断想起は統合-fts5-仮想表-judgment_fts--tool-search_judgments-で行う)（`search_judgments` で蒸留の材料参照）・[ADR-062](#adr-062-知識カード基盤に一本化する)（蒸留先＝`knowledge_cards` draft・triage で振り分け・意味検索 dedup）・[ADR-065](#adr-065-ai-advisor-の大画面化と知識ノート壁打ち導線)（`persist_card_ops_from_tool_runs`・propose_card/adjust_card_weight の既存経路）・[ADR-009](#adr-009-ai-の提案は承認制人間が承認するまで実行しない)（活性化は人間・draft 固定）・[ADR-013](#adr-013-投資方針-policy-は単一を育てる)/[ADR-015](#adr-015-システムプロンプトは不変-core--可変-policy-に分離)（policy/CORE は自動改変しない一線）・[ADR-029](#adr-029-生チャットは-fts5-索引しない揮発)（生チャットは教材にしない）・[ADR-058](#adr-058-llm-プロバイダ面別-providermodel-設定を-env-から-dbwebuisettings-へ移管する)（reviewer 面＝6 面目・面別 provider/model）・[ADR-018](#adr-018-夜間バッチの障害設計失敗は握って通知後続を止めない)（未設定=沈黙 skip・失敗=集約通知・カーソル保持）・[ADR-067](#adr-067-夜-digest-の注目シグナルを合流ゲートai-選別で作り直す)（`source` で呼び出し側ごとに配線する型）・[ADR-025](#adr-025-画面コンテキストは軽量ヒントのみチャットに数値を載せない)（数値は Python verbatim・push しない）・[data-model.md](data-model.md)（`reviewer:cursor` メタキー・`source='reviewer'` 値）・[advisor.md](advisor.md)（reviewer 面）。

## ADR-082: 投資家プロファイル記憶層は独立テーブル＋承認制の傾向メモで持つ（記述と規範を分離）

- **状況**: 自己改善ループ（`tasks/hermes-transfer-2026-07-02.md`）の ★4 テーマ C。★1 採点（[ADR-077](#adr-077-ai-の過去提案を市場結果で採点する自己改善ループproposal_outcomes--get_track_record)）・★2 想起（[ADR-078](#adr-078-判断ログ横断想起は統合-fts5-仮想表-judgment_fts--tool-search_judgments-で行う)）・★3 蒸留（[ADR-081](#adr-081-経験蒸留は-reviewer-面が採点済み-outcome-を知識カード-draft-に蒸留するtool-calling活動量ゲートmigration-不要)）はループを閉じたが、蒸留結果を**誰に返すか**の層が無い。AssetVane には `policy`（規範＝どうすべきか）はあるが「この投資家は**実際どう振る舞うか**（行動の癖）」の記述が無く、ユーザー訂正・行動傾向の記憶固定（グローバル CLAUDE.md の `lessons.md` に相当）が欠けている。`/grill-me` で設計を詰めた（保存先・更新主体・使い方・注入・粒度・信号を一問ずつ）。**現行 ADR-041 は「CORE の反追従＋ペルソナ層」で別物**であり、投資家プロファイルは本 ADR で新設する（過去の ADR-078/081 が「level=user プロファイル（★4・ADR-041 土台）」と呼んでいたのは本 ADR-082 の先取り）。
- **決定**:
  - **保存は独立の単一行ドキュメント `investor_profile`（id 固定・`body` 散文 1 枚＋`updated_at`）**（grill Q1）。`knowledge_cards`（[ADR-062](#adr-062-知識カード基盤に一本化する)）には**入れない**＝投資家のアイデンティティを市場知識の RAG プールに混ぜると意味検索が濁り weight/減衰の意味論も噛み合わない。幅広の構造化スキーマにもしない（＝「SQLite に住む 1 枚の markdown」）。repo file（`core_prompt.md` 系）にも置かない＝AI が実行時に自動更新する層は DB（[ADR-020](#adr-020-銘柄ドシエは-db知識はカードcore-はリポジトリmarkdown) の「AI が頻繁に更新＝DB／安定資産＝repo」の原則）。
  - **更新主体は AI が台帳から夜間蒸留 → 承認制 → 人間承認**（grill Q2）。夜バッチ profiler 面が取引台帳の行動事実から傾向メモを起票し、人間が承認して初めて本文に載る（[ADR-009](#adr-009-ai-の提案は承認制人間が承認するまで実行しない)）。
  - **粒度は増分**（grill Q3）。夜バッチは候補メモを既存 `proposals`（kind=`profile_note`・CHECK なしなので migration ゼロ）に起票し、人間が `/profile` で 1 件ずつ承認 → 承認メモだけが本文に**追記**される。全文再生成はしない＝毎晩の LLM 書き直しによる事実の脱落/ドリフトを避け、active 文書は**人間の承認/手編集でのみ**書き換わる。
  - **使い方は鏡・反追従**（grill・[ADR-041](#adr-041-ai-advisor-の-core-に反追従規律とペルソナ層を明示するリスク選好は-policy-のまま職業アイデンティティのみ固定) の反追従精神）。プロファイルは「行動の癖の記述」で、advisor は**それを打ち消す方向**にコーチングする（嗜好に迎合しない）。使い方の規律は CORE 要素③に散文で焼く（知識カードにしない＝[ADR-062](#adr-062-知識カード基盤に一本化する)「規律は CORE へ吸収」）。
  - **注入は軸2チャット＋軸1夜 AI の両方に第 3 層**（grill）。`build_messages` に `investor_profile` 引数を足し、POLICY（規範）の直後・知識カードの前に「記述の層」として system で入れる（CORE→POLICY→**投資家プロファイル**→知識カード→journal→…）。空文字は挿さない。蒸留する profiler 面自身には**重複回避の参照としてだけ**現行プロファイルを見せる（自己強化しない＝根拠は台帳事実のみ・instruction で縛る）。
  - **行動信号（v1）は quant 純関数が台帳から計算**（[ADR-014](#adr-014-ai-は計算しないtool-calling-原則rag-は後付け)/[ADR-016](#adr-016-手法はテスト済みコードで実装するai-にその場計算させない)）。①**手仕舞いの帰結**（各 SELL 約定の traded_at を起点に `compute_horizon_outcome`〔ADR-077〕を流用し 20/60 営業日後の実現/超過リターン＝「売った銘柄が戻る率」）②**ディスポジション効果**（FIFO 往復突合で勝ち/負けの平均保有日数差＝勝ち急ぎ・損塩漬け）③**繰り返す関心の集中**（buy の sector17 集中）。過学習足切り＝`min_samples`（既定 5）未満は「傾向」として断定しない。v1 は JP 台帳に焦点（US は機械的拡張で次段）。④AI 提案追従は v1 見送り。
  - **蒸留 face は新 7 面目 `profiler`**（reviewer 同型・[ADR-058](#adr-058-llm-プロバイダ面別-providermodel-設定を-env-から-dbwebuisettings-へ移管する)）。`FACES` tuple に 1 語足すだけ（seed migration なし・conftest 自動 seed・`/settings` 自動露出）。toolset は `PROFILER_TOOLSET = {get_track_record, search_judgments, propose_profile_note}` を allowlist で見せ、末尾で `persist_profile_notes_from_tool_runs` が profile_note だけ拾う（多重防御）。`propose_profile_note` は **allowlist_only**（ToolDef 新フラグ）＝面固有の書き込み Tool を chat/nightly に漏らさない。
  - **障害設計**（[ADR-018](#adr-018-夜間バッチの障害設計失敗は握って通知後続を止めない)）。profiler 面未設定＝沈黙 skip（ok=True・reviewer/tagger 同型）、活動量ゲート（新規 SELL＜閾値`profiler_min_new_sells` 既定 1）も ok=True、LLM ハード失敗＝ok=False で集約通知。カーソルは `fetch_meta('profiler:cursor')`（最後に蒸留した SELL の traded_at）で成功時のみ前進。digest に「🪞 投資家プロファイルの傾向メモ下書き N 件（/profile で確認）」の情報行。
- **理由**: 記述（誰か）と規範（policy）を**厳格分離**して [ADR-013](#adr-013-投資方針-policy-は単一を育てる) の緊張を回避＝プロファイルは第二の policy にしない・版管理化しない・policy 変更は承認制 `policy_change` のみ。台帳裏取りの行動フィードバック（手で書けない数字＝「急落で売った銘柄が後で戻る率」）が差別化価値で、これは DB（実行時書込）＋承認制でしか成立しない。既存機構（proposals 承認キュー・fetch_meta カーソル・reviewer 三層・検証 only Tool→persist 型・build_messages）を全面再利用し、新テーブルは `investor_profile` 1 つだけ。
- **代替案**: **(A) knowledge_cards に level=user 追加**→市場知識の RAG に人物像が混入・意味論不整合＝独立テーブル。**(B) repo の `.md` ファイルで持つ**→AI が実行時に git 追跡ファイルを書くと本番ラズパイの `git pull` デプロイと衝突＋[ADR-020](#adr-020-銘柄ドシエは-db知識はカードcore-はリポジトリmarkdown) 違反＝DB。**(C) 全文再生成→承認**→毎晩の LLM 書き直しで事実が脱落・承認が全 or 無の粗粒度＝増分メモ追記。**(D) パーソナライズ（嗜好に迎合）**→バイアスを餌やり・自己成就＝鏡/反追従。**(E) profiler に本文を直接書かせる**→承認制の一線を越える＝profile_note で人間承認。
- **段階**: 実装（2026-07-03・ATDD＝受け入れテスト先行・`/grill-me` で設計合意）。着手物＝① migration `0039_investor_profile`＋schema＋repo `investor_profile.py` ② quant `behavior.py`（sell_regret/disposition/concentration 純関数） ③ services `investor_behavior.py`（素材構築・カーソル・整形） ④ `FACES` に `profiler`（7 面化） ⑤ Tool `propose_profile_note`（schemas/handlers/registry・allowlist_only）＋`PROFILER_TOOLSET`＋`persist_profile_notes_from_tool_runs`＋`apply_profile_note`＋`resolve_proposal` の profile_note 分岐 ⑥ `advisor/profiler.py`＋job `distill_investor_profile`＋NIGHTLY_JOBS 挿入（distill_experience の後・通知系の前） ⑦ `build_messages` 第 3 層＋chat/nightly 配線 ⑧ router `/profile`（GET/PUT・/profile/notes）＋main 登録＋frontend `lib/api/profile.ts`・`app/profile/page.tsx`・nav ⑨ CORE 反追従一節＋digest 🪞 行 ⑩ テスト（quant/services/永続/承認追記/ゲート/注入順/face 未設定 skip/allowlist 露出/API/digest 行/FACES 7 化）⑪ docs 同期（本 ADR・[advisor.md](advisor.md)・[data-model.md](data-model.md)・[CLAUDE.md](../CLAUDE.md)・`tasks/hermes-transfer-2026-07-02.md`）。backend pytest green・ruff／pyright green・frontend biome／tsc green・migration を実 SQLite で head=0039 確認。**実データ運用（profiler 面を `/settings` で登録＋取引を入れて夜バッチが傾向メモを起票→承認→本文に育つ E2E）は ★1〜★3 と同じボトルネック（dev は LLM 未設定・提案 0 件）で次回運用時に確認**。スコープ外＝信号④（AI 提案追従）・自動 decay/再確認（v1 は日付付きメモ＋手編集で足りる）・US 行動信号・学習グラフ（★5）・cron（★6）。
- **関連**: [ADR-041](#adr-041-ai-advisor-の-core-に反追従規律とペルソナ層を明示するリスク選好は-policy-のまま職業アイデンティティのみ固定)（反追従・鏡の使い方の一線）・[ADR-013](#adr-013-投資方針-policy-は単一を育てる)（policy と別の単一行を育てる・第二の policy にしない）・[ADR-062](#adr-062-知識カード基盤に一本化する)（知識カードとの住み分け・規律は CORE）・[ADR-077](#adr-077-ai-の過去提案を市場結果で採点する自己改善ループproposal_outcomes--get_track_record)（`compute_horizon_outcome` を SELL に流用）・[ADR-081](#adr-081-経験蒸留は-reviewer-面が採点済み-outcome-を知識カード-draft-に蒸留するtool-calling活動量ゲートmigration-不要)（三層蒸留・活動量ゲート・toolset 制限の姉妹）・[ADR-009](#adr-009-ai-の提案は承認制人間が承認するまで実行しない)（承認制）・[ADR-058](#adr-058-llm-プロバイダ面別-providermodel-設定を-env-から-dbwebuisettings-へ移管する)（profiler 面＝7 面目）・[ADR-018](#adr-018-夜間バッチの障害設計失敗は握って通知後続を止めない)（未設定=沈黙 skip・カーソル保持）・[ADR-014](#adr-014-ai-は計算しないtool-calling-原則rag-は後付け)/[ADR-016](#adr-016-手法はテスト済みコードで実装するai-にその場計算させない)（行動信号は quant 純関数）・[ADR-025](#adr-025-画面コンテキストは軽量ヒントのみチャットに数値を載せない)（数値は verbatim）・[data-model.md](data-model.md)（`investor_profile` 表・`profiler:cursor`・`profile_note` kind）・[advisor.md](advisor.md)（profiler 面・注入第 3 層）。
