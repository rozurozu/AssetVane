...
LLM 接続は共通インターフェースのアダプタで抽象化する（OpenAI 互換 1 本・[decisions.md ADR-012](decisions.md)）。provider・API キー・ベース URL・model と**面別割当**（chat/nightly/dossier/tagger/triage/reviewer＝triage は知識カード審査〔[ADR-062](decisions.md)〕・reviewer は経験蒸留〔[ADR-081](decisions.md)〕）は **DB に保存し `/settings` の WebUI で編集する**（[ADR-058](decisions.md)）。複数の OpenAI 互換 provider（OpenRouter / OpenAI 直 / Claude / ローカル / Sakana）を登録し、面ごとに provider と model を割り当てられる（codex 経路は [ADR-073](decisions.md) で撤去）。
...
起動時に必須/任意キーを検証し、欠落時は**どの Phase で何が要るか**を明示するエラーを出す（[.env.example](../.env.example)）。LLM の provider/キー/model は env ではなく DB（`/settings` で登録・[ADR-058](decisions.md)）なので env 検証の対象外で、面の充足は `GET /llm/faces` の `configured` で見る。**migration 後、`/settings` で provider を登録するまで LLM は動かない**（シードしない）。
