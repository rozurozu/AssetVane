...
- **DB の単一所有者を FastAPI に固定** → SQLite の書き込みロック競合を原理的に回避（書き手は夜間バッチのみ）。スキーマも Python に一元化。
...
- **サービス**: `backend`（FastAPI/Python）＋ `frontend`（Next.js/Node）。SQLite はファイルなので **DB コンテナは作らず** `data/` を named volume で永続化（書き手は夜間バッチ 1 つ＝[ADR-002](decisions.md)）。
