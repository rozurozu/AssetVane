...
| `docs/decisions.md` | **ADR-001〜056。なぜそうしたかの全記録。最重要** |
...
| `docs/roadmap.md` | Phase 0〜7。**Phase 1〜4 ＋ Phase 6 ＋ Phase 7(A) ＋ Phase 7(B-1)（米株スクリーナー・提示専用・ADR-055）実装済み・Phase 5 は推論経路まで実装済み（backend＋frontend 通し検証済み・実ニュース取得＋Discord 通知＋日米業種リードラグ＋米株スクリーナーまで完了。Phase 2 backtest 画面接続は 2026-06-08 に完了）。Phase 5 の学習実測・Phase 7(B-2) 米株の FX/保有波及・ニュース意味検索系（ADR-045/051/052）・テーマタグ（ADR-050 改訂/ADR-056・全ユニバース grounded 事前タグ・段階 A/B/C）が残** |
