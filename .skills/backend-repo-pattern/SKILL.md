...
夜間バッチの大量 UPSERT や、API からの単発の単純な書き込み（取引 1 件・現金残高・外部資産 1 件など、1 文で閉じる書き込み）は、**repo 関数内で `with get_engine().begin() as conn:` を開いて commit まで完結**する。呼び出し側は rows を渡すだけ。バッチのジョブが毎回 begin を書かずに済む。
...
advisor 系のように、1 リクエストで複数テーブル（proposal ＋ journal ＋ llm_usage 等）を**まとめて atomic に**書くものは、**repo 関数は `conn` を受け取り execute だけ**して**自分では commit しない**。呼び出し側（router/service）が `with get_engine().begin() as conn:` で境界を所有し、複数 repo 呼び出しを 1 トランザクションに束ねる。
...
- **1 リクエストで複数表を atomic に**（中途半端な状態を残せない） → W2（呼び出し側が begin 所有）。
