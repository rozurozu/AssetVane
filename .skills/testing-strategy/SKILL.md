...
- **LLM も同じ。`client`/`temp_db` フィクスチャは `seed_llm_config()` で openai provider 1 行＋全 `FACES`（`for face in FACES`＝chat/nightly/dossier/tagger/triage/reviewer を自動追従）を一時 DB に seed** し、LLM 経路を openai（モック可能）に固定する（ADR-058/081・面別設定は env ではなく DB なので・旧 `settings.llm_provider_*=openai` monkeypatch の後継）。これがないと `resolve_face` が `FaceNotConfiguredError` で落ち `/chat` は 503 になる（ADR-058）。LLM 応答は `app.advisor.service.complete`（`get_client` を fake に差し替え or `service.complete` を monkeypatch）でモックする。面解決の振り分けは `test_llm_config.py`（resolve_face）/`test_engine_dispatch.py` がモックで別途検証する。
...
- [ ] LLM は openai 固定（`client` が provider を pin）＋ `service.complete` をモック
