"""services.news.build_news_context（ニュース3層文脈）の検証（ADR-044・ADR-014）。

担保すること:
- 3 層キー（stock/sector/market）が**常に存在**する（銘柄層が空でもキーは出る）。
- sector_label が JP_SECTOR_LABELS（TOPIX-17 和名）で解決される。
...
def test_build_news_context_resolves_sector_label_and_layers(temp_db) -> None:
    """銘柄の sector17_code を解決し JP_SECTOR_LABELS で和名化・3 層を揃える（ADR-044）。"""
    _seed_stock("7203", sector17_code="1622")  # 自動車・輸送機
    with get_engine().begin() as conn:
        repo.upsert_news(conn, [_row("https://s/1", level="stock", code="7203", source="news")])
        repo.upsert_news(conn, [_row("https://sec/1", level="sector", sector17_code="1622")])
        repo.upsert_news(conn, [_row("https://m/1", level="market", category="市況")])
...
def test_build_news_context_unknown_sector_skips_sector_layer(temp_db) -> None:
    """sector17_code 不明（銘柄に業種なし）ならセクター層は空・他層は返る（ADR-044）。"""
