...
_HTTP_TIMEOUT = 60.0
# スロットル間隔をプラン名から引く（ADR-008・ADR-061）。プランは DB（jquants_config.plan）で持ち、
# 秒数（レート上限の逆数の安全側）はここに固定する＝秒数を DB/env でお守りしない。Free は 5 req/分で
# **16 秒間隔なら任意の 60 秒窓で最大 4 req** に収まり下回る（13 秒は窓境界で 5 req に達し
# 実走でブロックを誘発した＝2026-06-04 本番投入の実測根拠）。Light（60 req/分）は 1 秒で足りる。
# standard（120 req/分）/ premium（500 req/分）は docs/jquants.md のレート上限から安全側の概算を置く
# （standard≈0.5s・premium≈0.12s。未実測なので実運用で要再確認）。未知プラン名は free に倒す。
_PLAN_INTERVALS: dict[str, float] = {
    "free": 16.0,
    "light": 1.0,
    "standard": 0.5,
    "premium": 0.12,
...
class JQuantsAdapter:
    ...
    def __init__(self, api_key: str, plan: str = "free", base_url: str = _BASE_URL) -> None:
        """api_key と契約プラン名を受け取って初期化する（ADR-061）。

        env は撤去したため（ADR-061）、接続値は DB から解決して渡される（services/jquants_config の
        build_jquants_adapter が単一点）。テストは fake キーを直接渡す（testing-strategy）。
        """
        ...
        # スロットル間隔はプラン名（引数 plan＝DB の jquants_config.plan）から引く（ADR-008/061・
        # _PLAN_INTERVALS）。未知プラン名（typo 等）は free（最も遅い＝最安全）に倒し、速くしすぎて
        # レート制限ブロックを誘発するのを防ぐ。気づけるよう warning を出す。
        plan = (plan or _DEFAULT_PLAN).strip().lower()
        if plan not in _PLAN_INTERVALS:
            logger.warning(
                "未知の J-Quants plan=%r。free 相当（%ss）に倒します（許容値: %s）。",
                plan,
                _PLAN_INTERVALS[_DEFAULT_PLAN],
                "/".join(_PLAN_INTERVALS),
            )
        self._min_interval = _PLAN_INTERVALS.get(plan, _PLAN_INTERVALS[_DEFAULT_PLAN])
