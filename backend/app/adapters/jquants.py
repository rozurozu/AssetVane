...
_HTTP_TIMEOUT = 60.0
# スロットル間隔をプラン名から引く（ADR-008・settings.jquants_plan）。秒数を env でお守りせず、
# プランごとの正しい値をここに固定する。Free は 5 req/分で、**16 秒間隔なら任意の 60 秒窓で最大
# 4 req** に収まり余裕を持って下回る（13 秒は窓境界で 5 req に達し実走でブロックを誘発した
# ＝2026-06-04 本番投入の実測根拠）。Light（60 req/分）は 1 秒で足りる（目安・実運用で要実測）。
# standard/premium は未実測のため未収載（必要時に実測して足す・YAGNI）。未知プラン名は free に倒す。
_PLAN_INTERVALS: dict[str, float] = {"free": 16.0, "light": 1.0}
_DEFAULT_PLAN = "free"
...
class JQuantsAdapter:
    ...
    def __init__(self, api_key: str | None = None, base_url: str = _BASE_URL) -> None:
        ...
        # スロットル間隔はプラン名（settings.jquants_plan）から引く（ADR-008・_PLAN_INTERVALS）。
        # 未知プラン名（typo・未収載の standard/premium）は free（最も遅い＝最安全）に倒し、
        # 速くしすぎてレート制限ブロックを誘発するのを防ぐ。気づけるよう warning を出す。
        plan = (settings.jquants_plan or _DEFAULT_PLAN).strip().lower()
        if plan not in _PLAN_INTERVALS:
            logger.warning(
                "未知の JQUANTS_PLAN=%r。free 相当（%ss）に倒します（許容値: %s）。",
                settings.jquants_plan,
