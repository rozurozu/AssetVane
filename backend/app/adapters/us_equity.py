...
def _parse_directory(text: str, *, symbol_col: str) -> list[dict[str, Any]]:
    ...
    for line in lines[1:]:
        ...
        # Yahoo 表記へ正規化（`BRK.A`→`BRK-A`）。$ 等を含む非普通株は None で除外（ADR-090）。
        normalized = _normalize_us_symbol(symbol)
        if normalized is None:
            continue

        rows.append({"symbol": normalized, "company_name": name or None, "is_etf": is_etf})
    return rows
...
# Yahoo Finance のティッカーとして扱える文字（英数字＋ハイフン）。NASDAQ Trader の `.` 区切りは
# ハイフンへ変換した後にこの集合へ収める（ADR-090）。
_YAHOO_SYMBOL_RE = re.compile(r"^[A-Za-z0-9-]+$")


def _normalize_us_symbol(symbol: str) -> str | None:
    """NASDAQ Trader のシンボルを Yahoo Finance 表記へ正規化する（ADR-090）。

    NASDAQ Trader directory はクラス株をドット（`BRK.A`）、優先株/ワラント/ユニット等を
    `$`/`+`/`=` 等の特殊記号（`BAC$L`）で綴るが、Yahoo Finance はクラス株をハイフン（`BRK-A`）で
    綴り、特殊記号付きシンボルは扱えない（`yf.Ticker('BF.A').info` が空 dict を返し、fetch_*
    が「bot 検知/レート制限の疑い」と誤診する＝ADR-090 の実害）。よって:

    - ドットはハイフンへ変換する（`BF.A` → `BF-A`）＝多クラス普通株を拾えるようにする。
    - 変換後に英数字・ハイフン以外を含む（`$`/`+`/`=` 等＝優先株/ワラント/ユニット等の非普通株）
      は None を返して除外する。Security Name ベースの `_is_non_common` をすり抜けた非普通株
      （名称に "preferred" 等を含まない綴り）を記号ベースで確実に落とす補完。

    普通株スクリーナー（Phase 7(B-1)）の対象は普通株＋クラス株のみで、優先株の指標
    （PER/EPS 等）は普通株と意味が異なるため拾わない（方針＝クラス株のみ正規化して回収）。
    """
    normalized = symbol.strip().replace(".", "-")
    if not normalized or not _YAHOO_SYMBOL_RE.match(normalized):
        return None
    return normalized
