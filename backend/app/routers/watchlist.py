"""watchlist の REST ルータ（GET/POST/DELETE /watchlist・spec §5.1 / docs/api.md §5）。

設計の真実: docs/phase-specs/phase4-spec.md §5.1・ADR-020/ADR-002/ADR-005。

HTTP 入出力のみを担う薄い層（ロジック・DB クエリ詳細は repo.py）。
- 読み取りは `Depends(get_conn)`、書き込み（add/remove）は repo が自前で `begin()` を所有する
  （add_watchlist / remove_watchlist が W1・UNIQUE 衝突は repo 側で do_nothing）。
- stale 判定（21 日超 or 未調査 → stale=true）はこの層の責務（repo は last_investigated_at を
  返すだけ・L-22）。
"""
...
# stale しきい値（spec §5.1・U-8/L-22 確定）。last_investigated_at がこれより古い、
# または未調査（None）なら再調査を促す（stale=true）。
_STALE_DAYS = 21
...
class WatchlistItem(BaseModel):
    ...
    stale: bool  # 21 日超 or 未調査 → 再調査を促す（backend 算出）
...
def _is_stale(last_investigated_at: str | None, *, now: datetime | None = None) -> bool:
    """last_investigated_at が 21 日より古い、または未調査(None)なら True（spec §5.1・L-22）。

    境界: 経過がちょうど 21 日なら stale ではない（「21 日超」=厳密に超過した時）。
    ...
    return (current - last).total_seconds() > _STALE_DAYS * 86400


def _row_to_item(row: dict[str, Any]) -> WatchlistItem:
    """list_watchlist の生 dict に stale を付与して WatchlistItem に整形（spec §5.1）。"""
...
def get_watchlist(conn: Connection = Depends(get_conn)) -> WatchlistResponse:
    """watchlist を company_name・last_investigated_at・stale 付きで返す（spec §5.1）。

    last_investigated_at は stock_dossiers JOIN（repo）、stale は 21 日しきい値で算出（L-22）。
    """
