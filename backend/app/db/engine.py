...
def _build_engine() -> Engine:
    ...
    # あわせて sqlite-vec 拡張をロードしてベクトル検索関数（vec_distance_cosine 等）を使える
    # ようにする（ADR-045: ニュース意味検索）。拡張は接続ごとにロードが要る。
    ...
    def _set_sqlite_pragma(dbapi_conn, _record):  # noqa: ANN001, ANN202
        ...
        # sqlite-vec のロード（ADR-045）。未導入・load_extension 不可の環境でも起動を壊さない
        # よう握る＝失敗時は警告ログのみで続行し、vec 機能だけオフになる（機能オフ耐性・ADR-018）。
        try:
        ...
        except Exception:  # noqa: BLE001 — vec 機能オフに留め他は動かす（ADR-018）
            logger.warning(
                "sqlite-vec のロードに失敗（ベクトル検索はオフで続行する・ADR-045）", exc_info=True
            )
