...
def run() -> JobResult:
    ...
    embedding 未設定なら ok=True・rows=0 で静かに skip（ADR-006）。1 バッチの埋め込み失敗は
    握って打ち切り（残りは翌晩・ADR-018）。near_dup 判定の失敗は握って degrade（候補提示は
    best-effort）。ジョブ境界の例外は握り ok=False で返す。
    """
    ...
    return JobResult(name="embed_themes", ok=True, rows=embedded, detail=detail)
