...
- 未配置（`<kind>-latest.json` 無し＝まだ別 PC で学習・rsync していない）→ **ok=True で skip**。
  runner の失敗通知（毎晩の偽アラート）は鳴らさない。ただし沈黙で「未配置」に気づけないのを避け、
  **notify_digest が digest の情報行『🤖 AI決算スコア: 学習モデル未配置』で毎朝可視化する**
  （ADR-066・失敗にはせず情報行で知らせる。pkl を配置すれば行は自動で消える）。
