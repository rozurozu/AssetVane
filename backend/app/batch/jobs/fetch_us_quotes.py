...
# 本当の障害検知として機能する（週末ごとの偽失敗を防ぐ）。さらに 02:00 JST 実行は米国ザラ場中で
# 当日バーが「途中足」のことがあるが、重ね窓の再取得により翌晩以降に確定足で UPSERT 上書きされ
# 自己修復する（同 C-2）。再取得は UPSERT で冪等（ADR-002）。
_REFETCH_OVERLAP_DAYS = 5


def _start_date(*, full_backfill: bool, today: str) -> str:
    """取得開始日を決める（fetch_index._start_date_for_symbol 同型・全銘柄共通カーソル）。

    full_backfill: today - BACKFILL_YEARS 年。
    差分: fetch_meta['us_daily_quotes'].last_fetched_date に _REFETCH_OVERLAP_DAYS 日重ねた地点
    （鮮度プローブ・C-1/C-2）。fetch_meta 不在/NULL なら full 相当（today - BACKFILL_YEARS）。
