...
取得アーキ（grill 確定・ADR-039(B)）:
  ...
  - シンボルを settings.us_quotes_batch_size ごとに分割し、1 バッチを `adapter.fetch_quotes_bulk`
    で **1 回の yf.download に束ねて** 取得し（per-symbol 取得＝約 3 時間が桁で短縮・ADR-055 当初
    設計「yf.download バッチ一括」への収束）、その結果を 1 トランザクション
    （`with get_engine().begin()`）で UPSERT する。HTTP 回数とスロットル待ちを symbol 数→バッチ数へ
    削減し、巨大トランザクション/長時間メモリ滞留も避ける。

部分失敗の握り（ADR-018・batch-pattern）: 1 シンボルが例外でも他を止めない。**全試行シンボルが
失敗（総崩れ）のときだけ ok=False**、1 本でも成功すれば ok=True（fetch_index.py 同型の割り切り）。
バッチ書き込み失敗は symbol 単位で failed に積み総崩れ判定と整合させる（#4）。カーソルは
**永続化に成功したバッチの最大 date** だけへ前進させる（未永続の date で進めると欠落する・#4）。
"""
...
def run(*, full_backfill: bool = False, adapter: UsEquityAdapter | None = None) -> JobResult:
    ...
    for batch in state.stop_aware(_batches(symbols, settings.us_quotes_batch_size)):
        ...
        try:
            ...
            # {symbol: [行...]}＝取れた symbol だけ。バッチ全滅は adapter が raise する。
            by_symbol = adapter.fetch_quotes_bulk(batch, from_=start, to=today)
        except Exception as exc:  # noqa: BLE001 — バッチ全滅を握り後続バッチを止めない（ADR-018）
            logger.info("fetch_us_quotes: バッチ %d 銘柄が全滅→スキップ: %s", len(batch), exc)
            failed.extend(f"{s}: {exc}" for s in batch)
            continue

        # 応答に含まれない symbol（部分欠損＝上場廃止/休場/シンボル誤り）も黙って落とさず計上する
        # （ADR-018 の精神・総崩れ判定の分母 attempted と整合）。
        failed.extend(
            f"{s}: 応答に含まれず（欠損/上場廃止の疑い）" for s in batch if s not in by_symbol
        )

        batch_rows: list[dict] = []
        batch_max: str | None = None  # このバッチで取れた最大 date（永続化成功時のみ採用・#4）
        for rows in by_symbol.values():
            ...
            if rows:
                ...
                rows_max = max(r["date"] for r in rows)
                if batch_max is None or rows_max > batch_max:
                    batch_max = rows_max
        ...
        if batch_rows:
            ...
            try:
                ...
                # 書き込み失敗は symbol 単位で failed に積む（総崩れ判定の分母 attempted と整合）
                # 1 エントリだけだと len(failed) < attempted となり、全バッチの書き込みが全滅しても
                # ok=True・無通知になってサイレントなデータ欠落に気づけない。
                failed.extend(f"{s}: バッチ UPSERT 失敗: {exc}" for s in batch if s in by_symbol)
                continue  # このバッチの max は採らない（未永続の date でカーソルを進めない）
            # カーソル前進は**永続化に成功したバッチの最大 date だけ**から積む（#4）。UPSERT に
            # 失敗したバッチの date でカーソルを進めると、未永続データを飛び越えてそのシンボル群の
            # 履歴を恒久欠落させる（差分は overlap しか遡らず次の手動 full_backfill まで戻らない）。
            if batch_max is not None and (max_date is None or batch_max > max_date):
                max_date = batch_max
