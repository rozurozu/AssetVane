...
取得アーキ（grill 確定・ADR-039(B)）:
  ...
  - 差分 = カーソル翌日から today まで（土日も from に含めてよい・yfinance が空で返す）。
...
def _start_date(*, full_backfill: bool, today: str) -> str:
    """取得開始日を決める（fetch_quotes._start_date 同型・全銘柄共通カーソル）。

    full_backfill: today - BACKFILL_YEARS 年。
    差分: fetch_meta['us_daily_quotes'].last_fetched_date の翌日。fetch_meta 不在/NULL なら
    full 相当（today - BACKFILL_YEARS）。日本株は J-Quants の日付一括だが米株は yf.download で
    [from, to] 範囲取得のため、from=翌日でよい（重なっても UPSERT で冪等＝ADR-002）。
    """
