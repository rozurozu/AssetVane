...
> **⏱ 進捗（2026-07-02）**: **テーマ A（★1 採点ループ）＝実装済み・自動検証済み（ADR-077）**。テーマ B〜E（★3〜★6）＋D-1 FTS5（★2）は**未着手**。詳細は末尾の「§7 実装ステータス」を参照。
...
| D-1（★2） | 判断ログ横断想起（FTS5 recall） | ⬜ 未着手 | — |
...
1. **テーマ B（★3）**: 採点済み outcome を教材に reviewer 面（FACES に triage 同型で 1 面追加）で `propose_card` draft を起票（承認制・過学習足切りを Python で）。A に依存＝ループを閉じる本丸。
2. **D-1（★2・FTS5 recall）**: A に非依存・低コストで journal n=1 帯域を拡張。独立着手可。
