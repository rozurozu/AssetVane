...
> **⏱ 進捗（2026-07-02）**: **テーマ A（★1 採点ループ）＝実装済み・自動＋実機 E2E 検証済み（ADR-077）**／**D-1 FTS5 recall（★2）＝実装済み・自動＋実機 E2E 検証済み（ADR-078）**。実機 E2E は dev の実 DB で採点 math・FTS 想起・get_track_record を実証（唯一「AI が Tool を呼ぶ」部分だけ LLM 未設定で未検証）。テーマ B〜E（★3〜★6）は**未着手**。詳細は末尾の「§7 実装ステータス」を参照。
...
## 8. ★3 テーマ B 設計ドラフト（2026-07-02・grill 途中で離席 → 推奨で仮置き・**要レビュー**）

grill-me で設計を詰める途中でユーザーが離席したため、**残りの分岐をずんだもんの推奨で埋めたドラフト**。戻ったら各決定を veto/微修正すれば実装（ATDD＋ADR-079）に入れる。**まだ承認されていない＝この時点でコードは書かない。**
